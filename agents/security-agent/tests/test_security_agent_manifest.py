...
from vaeloom_security_agent import SecurityAgent

MANIFEST_PATH = Path(__file__).parent.parent / "agent.yaml"


def test_security_agent_manifest():
    manifest = load_manifest_from_yaml(MANIFEST_PATH)
    assert manifest.agent_id == "security-agent"
...
async def test_security_agent_initialization():
    agent = SecurityAgent()
    assert agent.manifest.agent_id == "security-agent"
