...
class JobSearchAgent(BaseAgent):
    """Job Search Agent - Job board crawling, vacancy deduplication, and employer research."""
    ...
    async def run_step(self, context: AgentTurnContext, token: CancellationToken) -> ReActStep:
        ...
        return ReActStep(
            ...
            thought=f"Executing Job Search Agent plan for input: {context.user_prompt}",
            is_final=True,
            final_answer=f"Result from Job Search Agent",
        )
