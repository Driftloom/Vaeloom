...
class DocumentAgent(BaseAgent):
    """Document Agent - Document parsing, OCR text extraction, and semantic categorization."""
    ...
    async def run_step(self, context: AgentTurnContext, token: CancellationToken) -> ReActStep:
        ...
        return ReActStep(
            ...
            thought=f"Executing Document Agent plan for input: {context.user_prompt}",
            is_final=True,
            final_answer=f"Result from Document Agent",
        )
