...
| **Ollama Cloud (System 2)**      | `OLLAMA_API_KEY` (`07a8df...`), `OLLAMA_BASE_URL` | `https://ollama.com/v1` (`gemma4:31b`)                   | **PASS (HTTP 200)** | **CRITICAL BUG IDENTIFIED**: Bypassed in `llm_service.py:370-432` (`_resolve_api_key` has no `elif prov == "ollama":` branch). Calls fell back to Groq.                            |
| **Groq Cloud (Real-Time ReAct)** | `LLM_API_KEY` (`gsk_V4...`), `LLM_PROVIDER=groq`  | `https://api.groq.com/openai/v1` (`openai/gpt-oss-120b`) | **PASS (HTTP 200)** | **TRUNCATION BUG IDENTIFIED**: Generates ~96 reasoning tokens (`<think>`). In `router.py:_llm_classify_intent`, `max_tokens=64` caused finish_reason: length with empty text `""`. |
