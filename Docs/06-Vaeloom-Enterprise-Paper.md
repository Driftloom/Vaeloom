...
- **Define the enterprise vision** — what Vaeloom becomes after the MVP validates the core loop
- **Specify the full 28-agent architecture** — expanded from the MVP's eight agents
- **Document the complete enterprise memory taxonomy** — 22 memory types with lifecycle management
- **Design the enterprise security and consent model** — multi-tenant RBAC, permission engine, audit logging
- **Map the migration path from MVP to enterprise** — schema compatibility, agent growth, permission continuity
...
A person's knowledge, work, and career should not live scattered across forty disconnected apps, each with no memory of the others. Vaeloom's vision is a single, continuously learning intelligence layer that sits underneath everything a person does professionally and academically — one place that knows their education, their projects, their skills, their conversations, their applications, and their goals, and that uses that knowledge to act on their behalf, with their permission, for years.
...
A new, empty workspace is provisioned: an isolated memory namespace, a default folder taxonomy (Education, Career, Projects, Certificates, Research, Personal), and a blank knowledge graph. Nothing is pre-populated — Vaeloom never assumes structure the user hasn't created or content hasn't implied.

### 3.3 Memory initialization

The user is walked through a short, optional "seed" flow: upload a resume if one exists, connect one or two sources. This isn't required — Vaeloom works from zero — but it gives the memory system enough signal to be useful in the first session rather than the first week.
...
Over months and years, Vaeloom's value shifts from "organizes my stuff" to "knows my trajectory" — it can answer questions like "what should I learn next given where I want to go" or "how has my skill set actually changed in the last two years" with real, sourced evidence rather than generic advice, because it watched it happen.

---

## 4. Workspace

Vaeloom's workspace is the canonical home for everything a person creates or collects in their education and career. It is intentionally broad — the goal is that the user never has to ask "where should this go," because anything they touch has an obvious place.
...
Vaeloom's memory system is built in conversation with ideas from personal knowledge tools like Obsidian (bi-directional linking, a navigable graph of one's own notes) and from GraphRAG-style approaches to entity-and-relationship-grounded retrieval — without adopting either wholesale. The key structural difference: in those tools, the graph is built by the user's manual effort (linking notes, tagging entities). In Vaeloom, the graph is built automatically by agents reading the user's real activity, with the user able to view, correct, and override it, but never required to maintain it by hand.
...
- **Personal AI APIs** — letting a user expose narrow, permissioned slices of their own memory to other tools they trust, turning Vaeloom into infrastructure other products can build on top of.
...
**MCP (Model Context Protocol)** — the tool-calling shape Vaeloom's connectors and plugins are built around, enabling both consumption of external MCP servers and exposure of Vaeloom's own capabilities to other tools.
