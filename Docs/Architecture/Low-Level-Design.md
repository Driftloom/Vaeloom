...
> **Purpose:** Low-level design details for Vaeloom's core components
> **Status:** âœ… Upgraded to enterprise quality
> **Canonical source:** [`/Docs/Vaeloom-Complete-Documentation.md#13-implementation-blueprint`](../../Docs/Vaeloom-Complete-Documentation.md#13-implementation-blueprint)
...
graph TD
    ...
    subgraph AgentStructure["ðŸ¤– Agent Structure (ADR-006)"]
        direction TB
        S1["prompt.py<br/>Versioned system prompt"]
        S2["tools.py<br/>Declared tool list (MCP-shaped)"]
        S3["handler.py<br/>Core: retrieve â†’ reason â†’ output"]
        S4["permissions.py<br/>Read/write scopes (runtime-checked)"]
    end

    subgraph RequestFlow["ðŸ”„ Request Flow (LLD)"]
        F1["1. Client â†’ API Gateway"]
        F2["2. Auth Middleware â†’ Permission Engine"]
        F3["3. Route to Orchestrator or direct CRUD"]
        F4["4. Orchestrator â†’ specialist agent + RAG"]
        F5["5. Agent â†’ Model call (tool-calling)"]
        F6["6. QA Agent validation"]
        F7["7. Response + Event bus + Memory write"]
    end

    subgraph APIEndpoint["ðŸŒ API Endpoint Design"]
...
> **Diagram:** LLD showing **agent structure** (4-file pattern: prompt, tools, handler, permissions), **request flow** (7-step from clientâ†’APIâ†’authâ†’orchestratorâ†’agentâ†’QAâ†’response), and **REST endpoint design** (resource-oriented with workspaces, documents, resume, applications).
...
â”‚   â””â”€â”€ permissions.py # Read/write memory scopes (runtime-checked)
â”œâ”€â”€ memory_agent/
â”‚   â””â”€â”€ ...
â””â”€â”€ ...
```
...
```text
1. Client â†’ API Gateway â†’ Auth Middleware â†’ Permission Engine
2. Permission Engine â†’ Route to Orchestrator or direct CRUD
3. Orchestrator â†’ Select specialist agent â†’ Agentic RAG retrieval
4. Agent â†’ Model call (tool-calling) â†’ QA Agent validation
5. Output â†’ Response to client + Event bus publish + Memory write
```
...
|---------|-------------------|
| Writing endpoint specifications before defining the data model | Endpoints designed without a clear data model produce inconsistent APIs â€” similar resources get different response shapes, making client code complex and error-prone |
| Not including error response schemas in API designs | Clients need to know what error shapes to expect â€” an API that returns 400 with `{error: "invalid"}` for one endpoint and `{message: "Invalid", code: 400}` for another forces brittle error handling |
...
| API routes that bypass the permission engine for "internal" endpoints | An "internal" endpoint that doesn't check permissions is a backdoor â€” any service that discovers it can access data without authorization checks |

## Best Practices

| Practice | Rationale |
|----------|-----------|
| Define the data model (schemas, types, relationships) before API endpoints | A data model-first approach ensures consistent shapes across all endpoints â€” similar resources look similar; responses are predictable |
| Include error response schemas in every API endpoint specification | Every endpoint documents its possible error responses (400, 401, 403, 404, 500) with consistent shape â€” `{error: {code, message, details}}` for every error |
| Enforce the agent contract structure through code review and linting | An agent that doesn't follow the uniform structure (prompt.py, tools.py, handler.py, permissions.py) should fail code review â€” consistency is what makes 28 agents manageable |
| Apply permission checks on every route, including "internal" ones | An endpoint that is only called by another service still needs permission checks â€” internal services should have their own permissions scoped to what they legitimately need |

## Security

| Concern | Mitigation |
|---------|------------|
| API endpoint enumeration revealing resource IDs | Sequential resource IDs (workspaces, documents) allow enumeration â€” use ULID or UUID v7 for resource identifiers to prevent an attacker from guessing or enumerating valid IDs |
| Error responses leaking stack traces or query details | A 500 error that returns a stack trace or SQL query reveals internal system details â€” catch all errors at the API boundary and return sanitized error responses with a reference ID for log correlation |
| Permission check bypass via alternate route paths | If the same resource is accessible through multiple routes (e.g., `/workspaces/1/documents/2` and `/documents/2`), one path might skip permission checks â€” permission logic should be centralized, not repeated per route |

## Performance

| Concern | Guideline |
|---------|-----------|
| N+1 query patterns in REST API responses | An endpoint that returns a list of documents then queries each document's metadata individually creates N+1 database queries â€” use eager loading, batch queries, or GraphQL-style field selection to reduce query count |
| Serialization overhead for large response payloads | A single endpoint returning 1000+ documents with full entities serialized can take 500ms+ just in JSON serialization â€” paginate large responses (max 50 items per page) and use cursor-based pagination |
| Agent prompt loading time | Loading prompt.py, tools.py, and permissions.py from disk on every agent invocation adds file I/O overhead â€” cache loaded agent configurations in memory and invalidate only when the prompt version changes |
...
| LLD-FR-04 | Agent handler must follow retrieve â†’ reason â†’ output execution flow | P1 |
...
- [`/Docs/Vaeloom-Complete-Documentation.md#13-implementation-blueprint`](../../Docs/Vaeloom-Complete-Documentation.md#13-implementation-blueprint)
