...
> **Purpose:** Define performance targets and optimization strategy for Meridian
...
```bash
meridian perf test --endpoint /api/resume --concurrency 50 --duration 30s
```

### Check p99 latency

```bash
meridian perf stats --metric p99 --service api --window 5m
```
...
```bash
meridian perf budget set --page dashboard --lcp 2s --fid 100ms --cls 0.1
```
