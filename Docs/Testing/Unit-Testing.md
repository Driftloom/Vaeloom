...
> **Purpose:** Define unit testing standards for Meridian
> **Status:** 🆕 New
...
2. **Mock external dependencies**: Test uses `jest.mock('@meridian/api-client')` → mock returns controlled response → test verifies business logic without network calls → mock assertion verifies API was called with correct params → test completes in 50ms
...
Unit testing is a foundational practice in Meridian's quality assurance strategy. This document defines the standards, frameworks, and patterns for writing effective unit tests across the Meridian codebase — including backend services, frontend components, AI agent logic, and utility functions.

The unit testing strategy covers test structure (AAA pattern), mocking strategies, coverage thresholds, test naming conventions, and CI integration. Every module in Meridian must have corresponding unit tests that validate individual units of behavior in isolation.

Unit tests are the first line of defense against regressions in Meridian's rapidly evolving codebase. With multiple agents, services, and connectors being developed in parallel, comprehensive unit test coverage ensures that refactoring and feature additions don't break existing functionality.
