...
> **Purpose:** Define MCP integration architecture for Meridian
> **Status:** ✅ Upgraded to enterprise quality
> **Owner:** AI Team
> **Last Updated:** 2026-07-13
> **Canonical source:** [`/Docs/06-Meridian-Enterprise-Paper.md#53-mcp-architecture`](../../Docs/06-Meridian-Enterprise-Paper.md#53-mcp-architecture)

## Overview

The Model Context Protocol (MCP) integration positions Meridian in a dual role within the AI ecosystem: as a consumer that connects to external MCP servers (GitHub, Gmail, Slack) through a standardized client layer, and as a provider that exposes Meridian's own tools and memory as MCP endpoints for external IDEs and assistants. This architecture ensures that every tool call — whether internal or external — follows the same MCP-shaped format (name, description, inputSchema, outputSchema, requiredScope) and passes through the same Permission Engine.

This document defines the MCP client layer, server endpoints, permission bridging, tool registry, and health monitoring for Meridian's MCP integration. It serves AI engineers integrating third-party tools, platform engineers exposing Meridian's capabilities to external consumers, and security engineers auditing the permission model. All internal tools are MCP-shaped from day one, ensuring future-proof interoperability when native MCP transport stabilizes.

## Goals

- Enable Meridian agents to call any MCP-compatible external tool through a standardized client layer without custom integration
- Expose Meridian's memory graph and agent tools as MCP server endpoints for external IDE and assistant consumption
...
graph TD
    ...
    subgraph Consumer["🔌 Meridian as Consumer"]
        direction TB
        AGENTS["Meridian Agents"] --> MCP_CLIENT["MCP Client Layer<br/>Standardized tool calls"]
    ...
    subgraph Provider["🔓 Meridian as Provider"]
        direction TB
        EXT_CLIENT["External IDE / Assistant"] --> MCP_SERVER["Meridian MCP Server<br/>mcp.list_tools<br/>mcp.call_tool<br/>mcp.get_memory"]
...
> **Diagram:** Meridian plays a **dual role** with MCP. As a **consumer** (🔌), agents call external MCP servers through a standardized client layer — adding any MCP-compatible tool without custom integration. As a **provider** (🔓), Meridian exposes its own MCP server (`list_tools`, `call_tool`, `get_memory`) so external environments can query the memory graph under the same permission model. All internal tools are MCP-shaped from day one.

### Consumer: Meridian uses external MCP servers

Any MCP-compatible tool can be added as a connector without custom integration work.

### Provider: Meridian exposes its own MCP server

Meridian's memory and agents can be called from other environments (IDE, another assistant) under the same permission model.
...
| MCP transport channel security | MCP communications between Meridian and external servers must use TLS encryption; unencrypted MCP connections expose tool calls and data to network interception |
...
This document defines the Model Context Protocol (MCP) integration architecture for Meridian — covering Meridian's dual role as both MCP consumer (connecting to external MCP servers) and MCP provider (exposing Meridian's internal tools as MCP endpoints). Applies to all agent tool calls and external integrations across MVP and Enterprise. Out of scope: internal tool definition format (see [Tool-Calling.md](./Tool-Calling.md)), permission model specifics (see [Guardrails.md](./Guardrails.md)).
...
| MCP Server Layer | Expose Meridian tools as MCP endpoints | Fastify/Express HTTP server | Stateless; horizontal scaling |
...
### 2. Meridian MCP Provider Request

1. External client calls `mcp.list_tools` on Meridian MCP Server
...
```mermaid
sequenceDiagram
    participant AG as Meridian Agent
...
- [`/Docs/06-Meridian-Enterprise-Paper.md#53-mcp-architecture`](../../Docs/06-Meridian-Enterprise-Paper.md#53-mcp-architecture)
