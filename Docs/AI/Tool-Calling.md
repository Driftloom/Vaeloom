...
This document defines the tool-calling architecture for Vaeloom's AI agents — covering the tool definition format, permission checking, execution flow with retry logic, tool categories, and audit logging. Applies to all tool calls made by all agents across all environments. Out of scope: MCP integration specifics (see [MCP.md](./MCP.md)), permission model details (see [Guardrails.md](./Guardrails.md)), QA Agent validation (see [Guardrails.md](./Guardrails.md#qa-agent-architecture)).
