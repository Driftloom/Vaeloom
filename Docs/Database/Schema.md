...
> **Purpose:** Define the complete database schema for Meridian
> **Canonical source:** [`/Docs/Meridian-Complete-Documentation.md#11-database-design`](../../Docs/Meridian-Complete-Documentation.md#11-database-design), [`/Docs/Engineering/Implementation/02-database-schema.md`](../../Docs/Engineering/Implementation/02-database-schema.md)

## Overview

The database schema is the physical implementation of Meridian's relational data model — defining 9 core tables with complete column types, constraints (primary keys, foreign keys, NOT NULL, UNIQUE), and relationships that together store all user data, memory records, knowledge graph entities, application data, and audit logs. The schema is implemented as PostgreSQL DDL with UUID primary keys, workspace_id foreign keys for tenant isolation, JSONB for semi-structured memory content, and TIMESTAMPTZ for temporal data. The schema is managed through version-controlled ORM migrations (Prisma or Alembic) and must never be modified directly in production.
...
> **Diagram:** Entity-relationship diagram showing 9 core tables in Meridian's PostgreSQL schema. **users** → **workspaces** is the root hierarchy — every other table is scoped by `workspace_id`. **documents** have a version chain via **document_versions**. **entities** and **relationships** form the knowledge graph. **memory_records** link back to their source documents. **agent_actions** provides the append-only audit log.
