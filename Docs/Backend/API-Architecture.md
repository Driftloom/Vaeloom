...
> **Purpose:** Define the API architecture and design standards for Meridian
> **Status:** ✅ Upgraded to enterprise quality
> **Canonical source:** [`/Docs/Meridian-Complete-Documentation.md#132-api-structure`](../../Docs/Meridian-Complete-Documentation.md#132-api-structure)
...
sequenceDiagram
    ...
    C->>GW: HTTPS Request<br/>api.meridian.dev/v1/...
...
| JWT_ISSUER | Expected JWT issuer claim | https://auth.meridian.dev | Yes |
| JWT_AUDIENCE | Expected JWT audience claim | api.meridian.dev | Yes |
...
```typescript
// Meridian API client initialization
import { MeridianClient } from '@meridian/sdk';

const client = new MeridianClient({
  baseURL: 'https://api.meridian.ai/v1',
  apiKey: process.env.MERIDIAN_API_KEY,
});
```

```python
# Async API request with Meridian backend
import httpx

async with httpx.AsyncClient(base_url="https://api.meridian.ai/v1") as client:
    resp = await client.get("/workspaces", headers={"X-API-Key": "..."})
    print(resp.json())
```

```bash
# Curl example calling Meridian API
curl -X GET "https://api.meridian.ai/v1/workspaces" \
...
- [`/Docs/Meridian-Complete-Documentation.md#132-api-structure`](../../Docs/Meridian-Complete-Documentation.md#132-api-structure)
