...
The Build Prompts directory indexes agentic coding build prompts for Claude Code and Cursor, organized by implementation phase. These prompts guide the incremental construction of the Meridian platform from foundation infrastructure through deployment.
...
```bash
# Build prompts for Meridian
meridian build prompt --template agent-system
meridian build prompt --output ./prompts/assistant.md --vars role=assistant,tone=professional

# Validate prompt
meridian build validate --file ./prompts/assistant.md --check-length --check-tokens
```

```bash
# Prompt versioning and diff
meridian build version list --name assistant
meridian build version diff --name assistant --v1 1.0 --v2 1.1
```

```bash
# Prompt deployment
meridian build deploy --name assistant --version 1.1 --environment production
```
