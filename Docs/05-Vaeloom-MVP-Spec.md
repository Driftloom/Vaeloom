...
- **Define v1 product scope** â€” what's in and what's deferred to enterprise
- **Specify the eight-agent architecture** â€” agent roles, permissions, and autonomy defaults
- **Document the memory system** â€” six memory types, knowledge graph, and agentic RAG
- **Establish security and permission model** â€” least privilege, suggest-mode, audit trail
- **Provide a phased build plan** â€” five implementation phases

# A second brain for students and early-career professionals {#s1}

v1 scope Â· the buildable first version of the idea

**Scope note:** This is the v1/MVP spec â€” a buildable first version, not the full enterprise paper. A separate enterprise-grade vision document (multi-tenant, admin, SDK/marketplace, compliance, scale architecture) is the natural follow-up once this core loop is validated. Where useful, this doc flags which decisions are "v1 simple" vs "deferred to enterprise."
...
graph LR
    ...
    subgraph Process["âš™ï¸ Processing"]
        D["AI Ingestion<br/>Parse â†’ OCR â†’ Extract"]
        E["Memory Creation<br/>Knowledge Graph + Vector Store"]
    end

    subgraph Act["ðŸŽ¯ Action"]
        F["Agents Activate<br/>Organize, Resume, ATS, Job Search"]
        G["Suggestions & Automation"]
        H["Career Intelligence<br/>Resume â†’ Search â†’ Apply"]
    end

    subgraph Compound["ðŸ”„ Compounding"]
...
Reads each new/changed file, understands what it is, proposes a clean name and a destination folder, detects duplicates and version chains (`Resume_v2_final_FINAL.pdf` â†’ recognized as a version of `Resume.pdf`), extracts metadata and a short summary, and **writes everything it learns into memory** â€” this is the critical link the original outline implied but didn't say explicitly: organization isn't just filing, it's the main way memory gets populated.
...
A Job Search Agent searches connected platforms, ranks results by fit against the user's memory, and presents a shortlist. The user picks which ones to pursue. For each selected role, the agent tailors a resume and cover letter, shows a match-likelihood estimate and a gap list, and only proceeds to apply after approval. See Â§9 for the realistic constraints here.

### 4.8 Pages {#s14}

History/activity log, current-status dashboard, chat-with-agents, memory graph view, file/folder workspace with an in-app viewer, connectors page, providers page, schedule/important-dates page, settings. Full v1 sitemap in Â§10.

### 4.9 Gmail Agent

Checks Gmail on a schedule (default: once daily at 6 AM, configurable), classifies new mail, extracts dates and tasks, and surfaces anything time-sensitive on the Schedule page. See Â§8 for why a single daily pass needs a companion real-time path.
...
* **Hosted services** (Gmail, Drive, GitHub) â€” official OAuth + API integration, scoped tokens, read-only by default.
* **VS Code** â€” lightweight extension reporting workspace/git activity to the local agent.
* **Local folder** â€” a small desktop companion app that asks the OS for permission to one user-chosen folder, never broad disk access.
* **Plugin SDK (v1, minimal)** â€” a JSON schema for defining a new tool (inputs, outputs, scopes, auth type). Full marketplace/sandboxing is enterprise-paper territory.
...
5. Periodic consolidation â€” old, low-confidence, or superseded memories get compressed or archived.
...
* **Scheduled pass** (default 6 AM, configurable) â€” full inbox classification, daily digest, deadlines extracted to Schedule.
* **Lightweight real-time hook** (Gmail push notifications, not polling) â€” fires only for high-priority classifiers (interview, deadline-today, urgent-from-known-contact).
...
**v1 approach:** Use official APIs where they exist (full automation). Where they don't, the agent still does the valuable part â€” find, score, tailor â€” then deep-links the user to the listing with documents ready to attach, rather than scraping a form. Every application, auto or manual, logs to Career memory.
...
* **Least privilege by default** â€” every connector starts read-only; write/organize access is a separate, explicit grant.
* **Local folder access is scoped to one folder**, never the whole filesystem.
* **Nothing destructive without approval** â€” renames/moves are proposed first; deletions don't exist, files go to Archive.
* **Reversibility** â€” every Organization Agent action is logged with enough detail to undo it.
* **Encryption at rest** for documents and memory; OAuth tokens stored in a secrets manager, never in plaintext.
* **One clear data control** â€” "export everything" and "delete everything," visible and unconditional, from day one.

## 12. Gaps filled / additions beyond the original outline

1. **Suggest-mode-first design** â€” the single biggest trust risk in "an agent that organizes/renames/applies on your behalf" is doing the wrong thing autonomously.
2. **Undo/versioning for file operations** â€” cheap to build, expensive to skip.
3. **Memory consolidation** â€” without it the graph degrades into noise within a year of daily use.
4. **Realistic job-platform constraints** â€” covered in Â§9.
5. **A real-time path for the Gmail Agent** â€” covered in Â§8.
6. **Explicit autonomy settings per agent**, not a single global toggle.
7. **Cost/latency awareness** â€” debounce re-processing, don't re-embed on every file touch.
8. **A lightweight feedback loop** â€” corrections should feed back into memory so mistakes don't repeat.
9. **Desktop companion, not a browser extension**, for local folder access â€” a real build item to plan for.

## 13. Build phases

01**Foundation** â€” connectors (Drive, GitHub, local folder), ingestion pipeline, Organization Agent (suggest-mode), Memory Agent + knowledge graph + vector store, Workspace page, Memory Graph page.

02**Resume** â€” Resume Agent, ATS templates, ATS Agent, Resume & Career page.

03**Career** â€” Job Search Agent (API-based platforms first), application tracking, Jobs & Internships page.

04**Communication & time** â€” Gmail Agent (scheduled + push hook), Scheduler Agent, Schedule page.

05**Polish & autonomy** â€” Dashboard, History/audit log, per-agent autonomy settings, Settings/privacy controls.

## 14. What's deferred to the enterprise paper

Multi-tenant/org accounts, SSO/RBAC, full plugin marketplace, public API/SDK platform, the full 20-type memory taxonomy with provenance and explainability tooling, the complete enterprise connector list, education-edition vs. enterprise-edition product splits, monetization/business model, and infra-scale architecture. v1's job is to prove the core loop â€” ingest, organize, remember, assist â€” works and is trusted by real users first.

Vaeloom Â· MVP PRODUCT SPEC Â· v1
