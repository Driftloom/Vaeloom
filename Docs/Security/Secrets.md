...
> **Purpose:** Define secrets management strategy for Meridian
...
psql -h localhost -U admin -c "ALTER USER meridian WITH PASSWORD '$(cat new-secret.txt)';"

# 4. Restart services
flyctl restart apps/api

# 5. Verify
curl https://api.meridian.dev/v1/health
...
psql -h localhost -U admin -c "ALTER USER meridian WITH PASSWORD '$(cat new-db-password.txt)';"
...
DATABASE_URL="postgresql://dev_user:dev_pass@localhost:5432/meridian_dev"
...
async function getDatabaseUrl(): Promise<string> {
  const response = await secrets.getSecretValue({
    SecretId: `meridian/${process.env.ENVIRONMENT}/database-url`,
  });
