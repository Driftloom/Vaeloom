...
> **Purpose:** Common API usage examples for Meridian
> **Status:** 🆕 New
...
```bash
# Sign up
curl -X POST https://api.meridian.dev/v1/auth/signup \
  -H "Content-Type: application/json" \
  -d '{"email": "user@example.com", "password": "securepass123"}'

# Login
curl -X POST https://api.meridian.dev/v1/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email": "user@example.com", "password": "securepass123"}'
...
```bash
# Upload a document
curl -X POST https://api.meridian.dev/v1/workspaces/{id}/documents \
  -H "Authorization: Bearer $TOKEN" \
  -F "file=@resume.pdf"

# List documents
curl -X GET https://api.meridian.dev/v1/workspaces/{id}/documents?page=1&limit=20 \
  -H "Authorization: Bearer $TOKEN"

# Approve organization proposal
curl -X POST https://api.meridian.dev/v1/workspaces/{id}/documents/{doc_id}/organize/approve \
  -H "Authorization: Bearer $TOKEN" \
...
```bash
# Get master resume
curl -X GET https://api.meridian.dev/v1/workspaces/{id}/resume \
  -H "Authorization: Bearer $TOKEN"

# Generate variant
curl -X POST https://api.meridian.dev/v1/workspaces/{id}/resume/variant \
  -H "Authorization: Bearer $TOKEN" \
  -d '{"target_role": "SDE Intern", "company": "Google"}'

# Score against job description
curl -X POST https://api.meridian.dev/v1/workspaces/{id}/resume/score \
  -H "Authorization: Bearer $TOKEN" \
...
```bash
# Search memory
curl -X GET https://api.meridian.dev/v1/workspaces/{id}/memory/search?q=machine+learning \
  -H "Authorization: Bearer $TOKEN"
...
curl -X POST https://api.meridian.dev/v1/workspaces/{id}/chat/messages \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
...
The API Examples document provides ready-to-use curl commands covering Meridian's core API surfaces — authentication, documents, resume management, memory/knowledge graph queries, and agent chat interactions. Each example includes request parameters, response structures, and common error patterns to help developers integrate with the Meridian API quickly and correctly.

---

## Goals

- Provide copy-paste-ready API examples for all core Meridian endpoints
...
def generate_resume_variant(workspace_id: str, token: str, target_role: str, company: str):
    response = requests.post(
        f"https://api.meridian.dev/v1/workspaces/{workspace_id}/resume/variant",
