...
The event tracker is a lightweight JavaScript SDK (`@vaeloom/analytics`) deployed on the web app and an equivalent server-side SDK (`@vaeloom/analytics-node`) for backend services. Both enforce the same event schema, batch events locally, and flush to the ingress endpoint at configurable intervals (default 5 s or 100 events).
...
All endpoints are served from `https://analytics.Vaeloom.dev/v1` and require authentication.
...
import { Analytics } from '@vaeloom/sdk';
...
from Vaeloom.analytics import ReportBuilder
