...
| Metadata         | Value                                                                |
|------------------|----------------------------------------------------------------------|
| **Purpose**      | Enterprise product vision paper — Meridian at scale |
...
- **Define the enterprise vision** — what Meridian becomes after the MVP validates the core loop
...
```bash
meridian enterprise configure --sso saml --provider okta --tenant acme.edu
```

### Run a compliance audit report

```bash
meridian audit run --scope tenant --from 2026-01-01 --to 2026-07-13 --format json
```

### Register a plugin via SDK

```typescript
meridian.plugins.register({
...
|----------|-------------|
| [MVP Product Spec (v1)](01-Meridian-MVP-Spec.md) | The MVP this enterprise paper builds on |
