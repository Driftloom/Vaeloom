...
New workspace provisioned: isolated memory namespace, default folder taxonomy, blank knowledge graph. Nothing pre-populated — Vaeloom builds understanding from what the user actually connects.

### Stage 3: Memory Initialization

Optional seed flow: upload a resume, connect one or two sources. Not required — Vaeloom works from zero — but gives the memory system useful signal from day one.
