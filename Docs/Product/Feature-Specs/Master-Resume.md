...
- **Empty:** "Your resume is empty — Meridian is analyzing your files. This usually takes a few minutes." Progress indicator for first-time ingestion
...
```bash
# Get master resume
curl -X GET https://api.meridian.dev/v1/workspaces/{id}/resume/master \
...
# Export as PDF
curl -X GET https://api.meridian.dev/v1/workspaces/{id}/resume/export?format=pdf \
  -H "Authorization: Bearer $TOKEN" \
