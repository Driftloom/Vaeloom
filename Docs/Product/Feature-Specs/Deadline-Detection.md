...
"As a student juggling multiple internship applications with different deadlines and interview slots, I want Meridian to automatically find scheduling conflicts so that I never accidentally double-book an interview or miss an offer deadline."
...
curl -X GET https://api.meridian.dev/v1/workspaces/{id}/schedule/conflicts \
  -H "Authorization: Bearer $TOKEN"

# Resolve a conflict
curl -X POST https://api.meridian.dev/v1/workspaces/{id}/schedule/conflicts/{conflict_id}/resolve \
  -H "Authorization: Bearer $TOKEN" \
