...
"As a student juggling internship applications, I want Meridian to watch my inbox and surface deadlines and interview invitations so that I never miss an opportunity buried in promotional emails."
...
| **Out of Scope** | Auto-sending or auto-replying to emails (never granted); email composition (draft-only); full inbox management (Meridian is not an email client); email search within Meridian (use Global Search); calendar sync from email (use Calendar Connector); spam filter management |
...
```bash
# Get today's digest
curl -X GET https://api.meridian.dev/v1/workspaces/{id}/gmail/digest \
  -H "Authorization: Bearer $TOKEN"

# Generate draft reply
curl -X POST https://api.meridian.dev/v1/workspaces/{id}/gmail/messages/{msg_id}/draft \
  -H "Authorization: Bearer $TOKEN"
```
