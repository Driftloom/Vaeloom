...
> **Purpose:** Containerize every service, stand up staging and production environments, and wire CI/CD for automated deployments — making Meridian real for actual users.
...
jobs:
  ...
  deploy-staging:
    ...
    runs-on: ubuntu-latest
    steps:
      - run: flyctl deploy --app meridian-staging --image ghcr.io/meridian/api:${{ github.sha }}
      - run: npm run test:smoke -- --base-url https://staging.meridian.dev
  ...
  promote-production:
    ...
    runs-on: ubuntu-latest
    steps:
      - run: flyctl deploy --app meridian-prod --image ghcr.io/meridian/api:${{ github.sha }}
```
...
flyctl deploy apps/ai-service --image ghcr.io/meridian/ai-service:v1.1.0

# Verify rollback
curl -f https://api.meridian.dev/v1/health
npm run test:smoke -- --base-url https://api.meridian.dev
```
