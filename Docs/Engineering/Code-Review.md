...
> **Diagram:** Code review standards â€” **3 PR size tiers** (small/medium/large with reviewer count and SLA) â†’ **9-item checklist** (standards, tests, security, error handling, logging, docs, debugging, performance, compatibility) â†’ **5 AI-specific checks** (golden dataset, edge cases, prompt injection, output schema, fallback behavior).
...
| Mistake | Consequence |
|---------|-------------|
| Reviewing for style instead of correctness | Style preferences (formatting, naming) are better handled by linters and formatters â€” reviewers should focus on logic errors, security issues, and design problems |
| Leaving vague comments without specific line references | Comments like "this doesn't look right" without a line number or suggestion force the author to guess what's wrong â€” be specific or use the suggestion feature |
...
| Over-scoping review comments â€” demanding changes that aren't required | Requesting "nit" changes (preferred but not necessary) slows down delivery â€” distinguish between required and optional feedback clearly |
...
| Run the code locally for non-trivial changes | Static review misses runtime behavior â€” for logic changes, data migrations, or API changes, run the code to verify behavior |
| Use PR size as a signal, not a rule | A 600-line config change is less risky than a 200-line business logic change â€” size guidelines prioritize time-to-review, not risk |
| Distinguish blocking vs. non-blocking comments | Prefix with `blocking:` for must-fix and `nit:` or `suggestion:` for optional â€” the author knows what to action |

## Security Considerations

| Consideration | Mitigation |
|--------------|-----------|
| Reviewing authentication/authorization changes | Every auth or permission change must be reviewed for privilege escalation â€” a missing check in middleware can expose all user data |
...
| Data exposure in API responses | Review new endpoints for over-fetching or exposing sensitive fields â€” check that response DTOs don't include internal fields |

## Performance Considerations

| Consideration | Approach |
|--------------|----------|
| N+1 query detection during review | When reviewing changes that add related data access, look for loops making database queries â€” flag them for batch loading or JOIN optimization |
| Reviewing resource cleanup | Every new API endpoint or background job should have a cleanup path â€” unclosed connections, file handles, or event listeners leak memory |

## Workflows

1. **Author opens PR** with filled template â€” description, testing notes, related issues
2. **Size classification** â€” <100 lines (1 reviewer, 4h), 100-500 (1 reviewer, 24h), 500+ (2 reviewers, split recommended)
3. **CI validation** â€” lint, typecheck, test, build must all pass
4. **Reviewer assigned** â€” auto-assigned based on CODEOWNERS or manual request
5. **Reviewer performs checklist** â€” coding standards, tests, security, error handling, logging, docs, debugging, performance, compatibility
6. **AI-specific checks** â€” golden dataset, edge cases, prompt injection, output schema, fallback
7. **Author addresses feedback** â€” respond to every comment (fix or explanation)
8. **Approval + merge** â€” squash merge to develop (feature) or merge commit (main)
...
```typescript
// Example: PR description following the Vaeloom template
...
```python
# Example: AI-specific review â€” golden dataset validation
