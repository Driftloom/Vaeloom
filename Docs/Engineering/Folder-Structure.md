...
> **Purpose:** Define the monorepo folder structure for Meridian
> **Canonical source:** [`/Docs/Meridian-Complete-Documentation.md#131-monorepo-folder-structure`](../../Docs/Meridian-Complete-Documentation.md#131-monorepo-folder-structure)
...
The Meridian monorepo follows a structured layout that separates concerns across three application services (`apps/web`, `apps/api`, `apps/ai-service`), three shared packages (`packages/shared-types`, `packages/plugin-sdk`, `packages/ui-kit`), infrastructure configuration (`infra/`), and documentation (`docs/`). This document defines every directory's purpose, naming conventions, and depth limits so engineers can navigate the codebase without guesswork.
...
All Meridian engineers use this document as the canonical reference when creating new directories, modules, or agents. The structure is enforced through import-path lint rules and periodic audits, with a planned CI-enforced validation gate for Q3 2026.
...
import { Document, Workspace } from '@meridian/shared-types';
...
- [`/Docs/Meridian-Complete-Documentation.md#131-monorepo-folder-structure`](../../Docs/Meridian-Complete-Documentation.md#131-monorepo-folder-structure)
