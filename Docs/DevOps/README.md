...
The DevOps directory documents Meridian's deployment infrastructure, CI/CD pipelines, observability strategy, and infrastructure-as-code approach. It covers both the MVP deployment model using PaaS (Render/Fly.io) with Docker and the enterprise target architecture using managed Kubernetes.
...
| Deployment Architecture | [`/Docs/Meridian-Complete-Documentation.md#135-deployment-architecture`](../../Docs/Meridian-Complete-Documentation.md#135-deployment-architecture) | ✅ Good |
...
| `CONTAINER_REGISTRY` | Docker registry URL | `docker.io/meridian` | Yes (prod) |
...
- Establish a single source of truth for Meridian's deployment infrastructure, CI/CD pipelines, and observability strategy
...
- Third-party service configuration not managed by Meridian infrastructure
...
meridian deploy status --environment production
```
...
meridian config validate                          # Validate config files
meridian db migrate                               # Run database migrations
meridian health check --service all               # Health check all services
