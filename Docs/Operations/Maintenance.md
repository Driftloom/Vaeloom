...
> **Purpose:** Define maintenance procedures for Meridian
> **Status:** 🆕 New
...
- Categorize all Meridian maintenance activities into four types (routine, database, major, emergency) with defined frequency, duration, and user impact expectations
...
### In Scope
- Four maintenance types with frequency, duration, and user impact definitions for Meridian services: routine updates (weekly, <5 min, no impact), database maintenance (monthly, <30 min, slower queries), major upgrades (quarterly, <1 hour, brief downtime), and emergency (as needed, variable)
...
curl -X POST https://api.meridian.dev/v1/admin/maintenance/schedule \
...
kubectl exec -n meridian deploy/apps-api -- \
  psql -d meridian -c "VACUUM ANALYZE memory_records;"

# Check bloat after maintenance
psql -h $DB_HOST -d meridian -c "
