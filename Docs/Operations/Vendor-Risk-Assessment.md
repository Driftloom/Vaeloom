...
> **Purpose:** Define the vendor risk management program, assessment methodology, and data processing requirements for all Meridian third-party vendors
> **Status:** 🆕 New
> **Owner:** Security Team
...
Meridian relies on several external vendors for critical infrastructure, AI model access, authentication, and communication services. Each vendor is assessed for security posture, data handling practices, compliance certifications, and business continuity capabilities. Vendors are classified into risk tiers that determine assessment frequency and monitoring requirements.
...
2. **Data Classification:** Must acknowledge and handle data per Meridian's data classification policy
3. **Sub-processor Notification:** Must notify Meridian of any sub-processor changes
4. **Data Deletion:** Must delete customer data within 30 days of contract termination
5. **Breach Notification:** Must notify Meridian within 24 hours of confirmed breach affecting Meridian data
6. **Audit Rights:** Meridian reserves the right to audit vendor controls annually
...
| Require SOC 2 Type II or equivalent for Tier 1 vendors | Independent audit reduces Meridian's audit burden; provides objective evidence of controls |
| Document exit strategy before onboarding | Switching AI providers mid-crisis is harder than planning the switch in advance |
| Monitor vendor status pages proactively | Vendors announce incidents before they escalate to Meridian's customers; early warning enables proactive communication |
...
| Assuming vendor certifications cover all operations | SOC 2 scope may exclude specific services used by Meridian | Request SOC 2 with scope details; verify each service used is within scope |
...
| Ignoring vendor sub-processors | Vendor uses sub-processor that doesn't meet Meridian's security requirements | Review sub-processor list in DPA; require notification of changes; approve in advance |

## Security Considerations

| Concern | Mitigation |
|---------|-----------|
| Vendor data breach (Tier 1) | Customer data encrypted at rest and in transit; encryption keys controlled by Meridian (KMS); breach scope limited to data in use |
...
| Vendor data breach notification | Incident alert | Assess impact on Meridian data | Execute incident response per severity |
...
- Classify all Meridian third-party vendors into four risk tiers (Critical, High, Medium, Low) based on data sensitivity, customer data processing, and availability of alternatives
...
- Maintain current exit strategies for all vendors to ensure Meridian can migrate within 30 days if a vendor's security posture, pricing, or service quality deteriorates
...
### In Scope
- Complete vendor registry for all eight Meridian third-party vendors with risk tier, data classification, assessment frequency, DPA status, and certifications for each
...
### Out of Scope
- Meridian's own security architecture and compliance certifications (covered in Security documentation)
...
curl -s https://api.meridian.dev/v1/admin/vendors/status \
