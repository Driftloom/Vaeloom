...
| Metadata         | Value                                                                |
|------------------|----------------------------------------------------------------------|
| **Purpose**      | Documentation site view of Meridian — navigable web-style format |
...
- Provide a navigable, user-friendly overview of the complete Meridian product
...
what to apply to next. Meridian’s job is to make that re-assembly
...
the user to do the organizing. Meridian’s core bet is that the
...
every one of those other tools could then be built on top of. Meridian
...
Because the payoff compounds. The first week, Meridian is a decent
...
Every agent in Meridian — regardless of what it does — is built to
...
Memory in Meridian is not chat history. It’s a structured, durable,
...
**Expiration vs. permanence.** Working memory expires at
...
**What this project is.** Meridian is a second brain for
...
**Agent contract** — the shared structure every Meridian
...
shape Meridian’s connectors, plugins, and internal agent tools are all
built around.
...
MERIDIAN · COMPLETE PRODUCT DOCUMENTATION · BUILDS ON THE MVP SPEC & ENTERPRISE PAPER
...
```bash
meridian docs serve --port 3000
```

### Navigate to a specific section

```bash
meridian docs open --path /architecture/system-design
```

### Search documentation from CLI

```bash
meridian docs search "agent workflow"
```

### Export docs as a static site

```bash
meridian docs build --output ./_site --format html
```
...
| [Enterprise Product Vision](06-Meridian-Enterprise-Paper.md) | Enterprise-scale vision paper |
