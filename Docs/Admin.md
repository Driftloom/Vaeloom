...
> **Purpose:** Define the Meridian Admin Panel — capabilities, architecture, security, and operational workflows for platform administrators
> **Status:** 🆕 New
> **Owner:** Product Team
...
The Meridian Admin Panel is the central command center for platform administrators to manage users, workspaces, agents, billing, and system health. It provides a unified interface for day-to-day operations, compliance, and incident response across all tenants.
...
The admin panel is critical because it is the single point of operational control for the entire Meridian platform. A well-designed admin panel reduces operational overhead, ensures compliance through audited actions, and provides early warning for system issues. A poorly designed one creates security risks, operational bottlenecks, and support escalations.
...
graph TD
    ...
    subgraph Frontend["🖥️ Admin Panel Frontend"]
    ...
    subgraph Services["⚙️ Admin Services"]
...
The System Health dashboard provides real-time and historical visibility into the Meridian platform's operational status.
...
meridian admin user create --email admin@company.com --role super_admin
meridian admin workspace list --status active
meridian admin workspace delete ws_inactive_99 --force

# System administration
meridian admin system status
meridian admin system backup --output ./backups/
meridian admin system restore --file ./backups/meridian_2025-07-01.sql.gz
```

```bash
# Admin audit and reporting
meridian admin audit-log --since 24h --format json
meridian admin usage-report --workspace ws_abc123 --month 2025-06
meridian admin security rotate-keys --service api-gateway
```
...
- [`02-system-architecture.md`](./02-system-architecture.md) — Meridian system architecture overview
- [`06-Meridian-Enterprise-Paper.md`](./06-Meridian-Enterprise-Paper.md) — Enterprise feature catalog
