...
| RISK-MVP-P05-02 | Approval gate NOT enforced in agent loop — verified `14a1936`: `has_approval` bool param (handler.py:54/66), sole call site `orchestrator/loop.py:82-83` hardcodes `False`, `agent_approvals` decision never read back (wired-but-inert); no payload-hash column; immutability not structural | CRIT | ADR-021 verify at P07/P11; wire approval_manager into agent loop (consult persisted decisions); add payload_hash; immutable-decide; release blocker for send-capable path | Security | OPEN |
...
| EVD-MVP-P05-010 | User ratification of architecture baseline | MVP-P05-R08 | USER verdict 2026-08-15 (conditional approval) | VERIFIED |
| EVD-MVP-P05-011 | AMEND-2026-08-15 re-verification @ `14a1936`: approval single call site (loop.py:82-83, has_approval=False); RLS GUC never SET (0005_rls.py:29-30); 0002 duplicated across both migration systems; workload-id zero hits | MVP-P05-R01/R03 | `09-gate-2026-08-15.md` §Amendment | VERIFIED |
