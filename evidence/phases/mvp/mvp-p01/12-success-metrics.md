...
> explicit non-goals. **Status:** V2.0 - upgraded 2026-08-16 to reflect
> telemetry gaps. Analytics service only tracks memories_created, agents_run,
> tokens_used. Metrics M-01/M-03/M-04/M-05 require telemetry not yet built.
> Approval gate hardcoded OFF (`loop.py:83`). QA gate exists but has no metric.
> **Owner:** Product Manager / Platform / Privacy. **Prompt reference:**
> —1/—3/—5/—7/—9/—12/—19/—20. **Rule:** targets are `TO_BE_DECIDED` (with owner)
> unless the target is spec-derived (prompt —3 fixed decisions). No runtime
> measurement exists in DISCOVERY - all runtime metrics are `NOT_EXECUTED`;
> runtime ownership and instrumentation land in P05/P15/P17 as noted.

## 1. Metrics register

| # | Metric | Domain | Definition + formula | Owner | Measurement method | Target | Status |
| ---- | --------------------------------------- | --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- | ------------ |
| M-01 | Task success rate | Outcome | % of end-to-end user task flows (ingest a doc, tailor a resume, approve an action, set a reminder) completed without user correction or abandonment; `completed_flows / started_flows` | Product | TELEMETRY NOT BUILT - analytics service only tracks memories_created/agents_run/tokens_used (`services/analytics_service.py`); task-level flow tracking required. Cohort task protocol (RB-04) also needed. | TO_BE_DECIDED (Product, at cohort launch) | NOT_EXECUTED |
...
| M-03 | Repeat-input reduction | Quality/outcome | Decline in duplicate entry events per user per week vs. adoption baseline; `(baseline_rate - post_rate) / baseline_rate` | Product | TELEMETRY NOT BUILT - requires duplicate-entry event tracking not in analytics service. Also blocked by MemoryAgent scope: only reads/writes2 of22 memory types (`memory_agent/handler.py:25-27`); remaining20 types not wired into extraction pipeline. | TO_BE_DECIDED (>=30% decline candidate, H-01; feeds BQ-06b) | NOT_EXECUTED |
| M-04 | Wrong-memory incidents | Trust/quality | Count of user corrections of memory per week + incidents where wrong memory caused user-visible harm (missed deadline, wrong application content); `corrections/week` and `harmful_incidents` | AI Product Lead / UX | TELEMETRY NOT BUILT - requires correction event tracking and harm incident logging not in analytics service. The Memory model (`models/schema.py:212`) has `supersedes_id` for versioning but no correction event pipeline. | TO_BE_DECIDED (harmful incidents target 0 candidate) | NOT_EXECUTED |
| M-05 | Approval-confusion rate | Trust | % of approval prompts where the user mis-approves, seeks help, or abandons the flow; `confused_events / approval_prompts` | Privacy / UX | BLOCKED - approval gate not implemented (`loop.py:83` hardcodes `has_approval=False`). No approval prompts exist to measure. Requires approval infrastructure built first. Moderated study (VB-03) also needed. | TO_BE_DECIDED (H-03; BQ-06a feeds churn link) | NOT_EXECUTED |
...
| M-19 | QA gate rejection rate | Quality | % of agent outputs rejected by QA gate before delivery; `qa_rejections / total_agent_outputs`. Also track `best_effort_after_retries` flag rate. | AI Product Lead | CODEBASE_VERIFIED - QA gate exists (`router.py:278-296`); requires event logging around `QAAgent.validate()` calls. | TO_BE_DECIDED | NOT_EXECUTED |
...
- `docs/phases/mvp-p01/09-problem-statement.md` (PS-01..04, constraints
 S-01..10)
- `docs/phases/mvp-p01/11-value-risk-hypotheses.md` (H-01..H-09)
...
- `apps/api/src/backend/services/analytics_service.py` (current telemetry:
 memories_created, agents_run, tokens_used only)
