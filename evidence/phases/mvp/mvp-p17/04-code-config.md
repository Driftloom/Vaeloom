# MVP-P17 — 04. Code and Configuration
...
Per phase rule: **Trace user→workspace→job→agent→tool→model→memory→approval→action** — every hop carries `X-Correlation-ID` + `X-Tenant-ID`/`X-Workspace-ID` + `tenant_id`/`user_id` ContextVar into logs/traces/metrics, redacted via `_redact` 9 keys before persistence.
...
| `apps/api/src/api/logging.py:7` | `_REDACT_KEYS` 9 keys password/password_hash/token/access_token/refresh_token/authorization/cookie/api_key/secret → `[REDACTED]` recursive dict/list, 3 ContextVars `correlation_id_var/tenant_id_var/user_id_var` | PII/secret redaction before log persistence | `logging.py:7` ~60 lines |
...
| `apps/api/src/api/main.py:170` | Middleware chain `RateLimit 100rpm` → `Tenant inner Auth` → `CSRF` → `SecurityHeaders` → `CorrelationID` → `RequestLogging` → `APIVersion` → `PromptInjection` → `Idempotency` → `Metrics` → `IPAllowlist` → `CORS outermost` | Correct RLS + correlation before metrics | `main.py:170-196` |
...
| `infra/ops/INCIDENT-RESPONSE.md:1` | SEV1 15m SEV2 30m SEV3 2h SEV4 next-day + 7-day rotation Mon 09:00 UTC + channels vaeloom-alerts/incidents/eng/status.vaeloom.app + Detect→Triage<5m→Mitigate<30m + rollback/scale/flag/restore/failover/WAF/restart `make` cmds | Incident command | `INCIDENT-RESPONSE.md:1` ~180 lines |
...
| `LOG_FORMAT` | `json` prod else `pretty` `logging.py:83` `service_environment` local/dev/test→pretty | `StructuredJsonFormatter` vs `PrettyFormatter` |
...
| `SLO` | `p50<100 p95<500 99.9% error<1% RPO1h RTO15m` `slo-dr.md:1` + `capacity-model.md:12` 20 RPS headroom 60%→50 RPS | Burn 0.04% <0.1% budget |
...
- `promtool check rules infra/ops/monitoring/alerts.yml` → SUCCESS 9 rules 3 groups + `infra/monitoring/alerts/vaeloom-alerts.yml` 4 PASS =13 total
...
- `python -c "from api.logging import _redact; print(_redact({'password':'x','token':'y','ok':'z'}))"` → `{'password':'[REDACTED]','token':'[REDACTED]','ok':'z'}`
- `k6 run --vus 10 --duration 30s infra/ops/load-test/k6-script.js` → p95 115ms <200 budget PASS gates deploy
