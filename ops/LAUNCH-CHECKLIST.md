...
- [ ] **Secret scanning** — verify no secrets committed via `git secrets` / `trufflehog`
- [ ] `.env.production` populated from `.env.production.template`, all values filled
...
- [ ] ACM certificate issued for domain + `*.domain` in us-east-1 (for CloudFront)
...
- [ ] Backend tests pass: `cd apps/backend && python -m pytest tests/ -q`
