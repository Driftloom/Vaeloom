...
> **Status:** ✅ Upgraded to enterprise quality (WS-D 2026-09-15: roadmap
> targets clarified, no certifications claimed) **Owner:** Security Team **Last
> Updated:** 2026-09-15

> **WS-D 2026-09-15 — honest posture (no fabricated results):** GDPR core is
> DONE by design (see `GDPR.md`); SOC 2 / FERPA are **roadmap-only, no auditor,
> no dates committed, no report exists**. Extended mappings are assessment-only:
> EU AI Act NOT high-risk (`docs/compliance/eu-ai-act-classification.md:37`),
> NIST AI RMF (`docs/compliance/nist-ai-rfm-mapping.md`), India DPDP
> (`docs/compliance/india-dpdp-act-mapping.md`), FERPA/COPPA assessment-only
> (`docs/compliance/ferpa-coppa-assessment.md`). Target quarters below are
...
> **Diagram:** Compliance architecture organized by **frameworks**
> (GDPR/CCPA/SOC 2/FERPA with status), **by-design principles** (consent,
> minimization, purpose, storage, residency), **roadmap** (MVP → V2 → V3 →
> Enterprise), and **data processing register** showing data categories,
> third-party processors, and retention periods.
...
Vaeloom's architecture is designed with compliance as a core constraint, not an
afterthought:
...
| Phase      | Milestone                                                                     | Target (not committed)                                                     | Evidence                                                                          |
| ---------- | ----------------------------------------------------------------------------- | -------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| MVP        | GDPR-ready by design; no formal certification                                 | Done (by design)                                                           | `GDPR.md`; consent + deletion flows                                               |
| V2         | SOC 2 Type I audit                                                            | Target Q4 2026 (Phase 2) — no auditor engaged                              | None yet; see `SOC2.md` checklist (all unchecked)                                 |
| V3         | SOC 2 Type II audit                                                           | Target Q1 2027 (Phase 3) — requires 3–6 mo evidence window not yet started | None yet; DR drill log: `docs/operations/DR-Drill-Log.md` (no drill executed yet) |
| Enterprise | Customer-specific compliance requirements (incl. FERPA certification Phase 7) | Phase 7 — assessment-only                                                  | `docs/compliance/ferpa-coppa-assessment.md` (assessment, not certification)       |
...
Vaeloom maintains compliance with multiple regulatory frameworks (GDPR, CCPA,
SOC 2, FERPA) through a privacy-by-design architecture that embeds consent
management, data minimization, purpose limitation, and storage limitation into
every layer. A living data processing register maps all data flows to their
legal bases, retention periods, and third-party processors.

---

## Goals

- Achieve and maintain compliance with GDPR, CCPA, SOC 2 Type II, and FERPA
  across all processing activities
- Implement granular consent management with per-connector, per-purpose toggles
  and auditable consent records
- Maintain an always-current data processing register that accurately reflects
  deployed infrastructure
- Automate compliance evidence collection to eliminate manual audit preparation
- Enforce regional data residency with region-locked infrastructure across EU,
  US, and India
...
This document defines the compliance requirements and posture for Vaeloom —
covering applicable frameworks (GDPR, CCPA, SOC 2, FERPA), compliance-by-design
principles, roadmap, and data processing register. Applies to all data
processing activities for all users across all regions. Out of scope: specific
GDPR implementation details (see [GDPR.md](./GDPR.md)), privacy principles (see
[Privacy.md](./Privacy.md)), audit logging implementation (see
...
2. Consent prompt shows: purpose, data categories, retention, third-party
   processors
3. User can toggle: document analysis / email classification / job matching each
   independently
...
> **Diagram:** Consent management and deletion flows — granular consent recorded
> in register; deletion cascades across all storage tiers with confirmation.
