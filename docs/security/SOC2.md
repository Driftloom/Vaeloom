...
> **Purpose:** Define Vaeloom's SOC 2 Type II compliance roadmap — Trust Service
> Criteria mapping, control implementations, audit readiness, and certification
> timeline **Status:** 📋 Spec-Only / Planned (Phase 7) — no auditor engaged, no
> certification achieved **Owner:** Security Team **Version:** 1.1 **Last
> Updated:** 2026-09-15 (WS-D) **Dependencies:**
> [`Compliance.md`](./Compliance.md),
> [`Security-Architecture.md`](./Security-Architecture.md),
> [`Audit-Logs.md`](./Audit-Logs.md), [`IAM.md`](./IAM.md),
> [`../Enterprise/Enterprise-Architecture.md`](../Enterprise/Enterprise-Architecture.md)
> **Implementation Status:** 📋 Spec Only

> **WS-D 2026-09-15 — honest status (no fabricated results):** TSC CC1–CC9
> mappings below are **design targets, not operating controls**. No external
> auditor has been selected, no observation period has started, no Type I/II
> report exists. Target quarters (not commitments): readiness/gap remediation
> **Q4 2026 (Phase 2)**, Type II observation + audit **Q1 2027 (Phase 3)** per
> `Compliance.md` Future Improvements. GDPR core is DONE (see `GDPR.md`);
> extended mappings are assessment-only: EU AI Act NOT high-risk
> (`docs/compliance/eu-ai-act-classification.md:37`), NIST AI RMF mapping
> (`docs/compliance/nist-ai-rfm-mapping.md`), India DPDP mapping
> (`docs/compliance/india-dpdp-act-mapping.md`), FERPA/COPPA assessment-only
> (`docs/compliance/ferpa-coppa-assessment.md`). Evidence checklist below marks
> what exists vs what is missing; unchecked boxes are intentional.

## Overview

SOC 2 (System and Organization Controls 2) is an audit framework that verifies a
service organization's controls meet the Trust Service Criteria (TSC) for
security, availability, processing integrity, confidentiality, and privacy. SOC
2 Type II certification is a prerequisite for selling to most enterprise
customers. This document defines Vaeloom's roadmap to certification, maps each
TSC to implemented controls, and establishes the evidence-collection strategy.
...
- All five Trust Service Criteria (Security, Availability, Processing Integrity,
...
- General security architecture (see
  [`Security-Architecture.md`](./Security-Architecture.md))
...
> **Diagram:** SOC 2 Trust Service Criteria. Security (Common Criteria CC1-CC9)
> is required for all audits. Availability, Processing Integrity,
> Confidentiality, and Privacy are optional categories Vaeloom includes.
...
| CC7.4 (Recovery)            | Backup + DR                        | RPO <1h; RTO <4h (**committed SLA ceiling** — cf. DR runbook engineering targets Critical RTO ≤1h / RPO 5m; see reconciliation note below); annual DR drill | Backup logs; DR test report (**none yet** — see `../operations/DR-Drill-Log.md`: no drill executed yet) |
...
> **WS-D 2026-09-15 — evidence honesty:** all boxes below remain **unchecked**
> as of this update (no 3–6 month Type II evidence window, no pen test in last
> 12 months, no DR drill — see `docs/operations/DR-Drill-Log.md`). Check a box
> only with dated evidence (report + artifact link).

- [ ] All Common Criteria (CC1-CC9) controls implemented and documented
- [ ] Evidence collection automated (logs, configs, reports)
- [ ] At least 3-6 months of operating evidence (SOC 2 Type II requires
      observation period)
...
> **Diagram:** SOC 2 certification timeline. ~12 months from gap assessment to
> certified report. The observation period (3-6 months of evidence) is the
> longest phase.

> **WS-D 2026-09-15:** the Gantt dates above are illustrative (starting
> 2026-08-01) — no auditor-fieldwork dates are committed. Honest targets:
> gap/remediation **Q4 2026**, evidence observation + audit **Q1 2027** (targets
> only). See also RTO/RPO reconciliation in `docs/DISASTER_RECOVERY.md`
> (Critical RTO ≤1h / RPO 5m engineering targets) vs CC7.4 row below (RPO <1h /
> RTO <4h) — CC7.4 states the **committed SLA ceiling**, DR runbook states the
> **engineering target**; neither is currently evidenced by a dated drill (see
> `docs/operations/DR-Drill-Log.md`: no drill executed yet).

## Gap Analysis

> **RTO/RPO reconciliation (target vs committed SLA, WS-D 2026-09-15):**
> `docs/DISASTER_RECOVERY.md` states **engineering targets** (Critical RTO ≤1h /
> RPO 5m — what procedures aim for). CC7.4 above states the **committed SLA
> ceiling** (RPO <1h / RTO <4h — the contractual maximum). The runbook target
> fits inside the SLA ceiling by design. Neither number is evidenced: no dated
> drill report exists (`docs/operations/DR-Drill-Log.md`: no drill executed
> yet).
...
- [`Security-Architecture.md`](./Security-Architecture.md) — security
  architecture
- [`Audit-Logs.md`](./Audit-Logs.md) · [`Audit-Policy.md`](./Audit-Policy.md) —
  audit logging
