...
> **Purpose:** Define secrets management strategy for Vaeloom **Status:** ‚úÖ
...
```mermaid
sequenceDiagram
    participant DEV as üë®îçüíª Developer / CI
    participant SM as ü--ÑÔ∏è Secrets Manager
    participant APP as üöÄ Application
    participant TGT as üéØ Target Service<br/>(DB / API / Redis)
    participant MON as üìä Monitoring

    Note over DEV,MON: ‚îÄ‚îÄ 1. Secret Creation ‚îÄ‚îÄ
    ...
    Note over DEV,MON: ‚îÄ‚îÄ 2. Secret Rotation ‚îÄ‚îÄ
    ...
    alt Rotation failed
        SM->>MON: üö® Rotation failure alert
        SM->>SM: Keep previous version active<br/>Retry rotation in 5 min
    end

    Note over DEV,MON: ‚îÄ‚îÄ 3. Secret Revocation ‚îÄ‚îÄ
    ...
    alt New secret available
        ...
        APP->>MON: üö® Critical: app has no valid secret
        APP->>APP: Graceful shutdown / degraded mode
    end

    Note over DEV,MON: ‚îÄ‚îÄ 4. Secret Audit ‚îÄ‚îÄ
...
| Mistake                                                                  | Consequence                                                                                                                                                                                                                      |
| ------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Loading secrets from environment variables at runtime instead of startup | Reading `process.env` on every request means a configuration change could expose secrets to a concurrent request ó load all secrets once at application startup and cache in memory                                              |
| Storing secrets in version control                                       | A `.env` file or hardcoded API key committed to git is exposed to every developer who has access to the repository ó use `.env` only for local development, never commit it, and scan for accidental commits with pre-push hooks |
| Using the same secret across environments                                | A development API key that gets exposed in CI logs exposes the production key if they're the same ó use separate secrets per environment with different values and access controls                                               |

## Best Practices

| Practice                                                                      | Why                                                                                                                                                                                 |
| ----------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Load secrets once at startup, cache in memory, never read from env at runtime | Reading from environment variables at runtime introduces timing windows and makes it harder to audit secret access ó load during initialization and swap references on rotation     |
| Use a secrets manager with automatic rotation for production                  | Cloud secrets managers (AWS Secrets Manager, GCP Secret Manager) provide encryption, access logging, and rotation ó don't manage secrets in configuration files                     |
| Implement a secret rotation strategy with verification steps                  | After rotating a secret, verify that all services are using the new value before retiring the old one ó use a blue/green approach where old secrets are valid during a drain period |

## Security

| Concern                                                 | Mitigation                                                                                                                                                                                         |
| ------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Secrets exposed through error messages and stack traces | An unhandled exception that includes a connection string or API key in the error message leaks to log aggregation ó sanitize all error outputs to redact known secret patterns                     |
| Secrets in application memory dumps                     | A crash dump of a service that loaded secrets into memory can leak them ó use isolated memory for secret storage and zero out after use where possible                                             |
| Privilege escalation via secrets manager access         | A developer with access to the staging secrets manager could read production secrets if permissions aren't scoped ó enforce strict IAM separation between environments and audit all secret access |

## Performance

| Concern                                      | Mitigation                                                                                                                                                                                         |
| -------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Secrets manager API latency on every request | Calling AWS Secrets Manager or GCP Secret Manager on every request adds 50-200ms ó load secrets once at startup into memory and refresh only on rotation notifications                             |
| Secret rotation causing connection storms    | Rotating a database password causes all service instances to reconnect simultaneously ó implement staggered rotation where instances reconnect over a drain window, not all at once                |
| Encryption overhead for secrets at rest      | Cloud secrets managers encrypt at rest by default, but custom secret storage (e.g., encrypted files) adds CPU overhead ó use managed secrets manager services that handle encryption transparently |
...
| Concern                                      | Approach                                                                                                                                                                                           |
| -------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Secrets manager API latency on every request | Calling secrets manager on every request adds 50-200ms ó load secrets once at startup into memory and refresh only on rotation notifications                                                       |
| Secret rotation causing connection storms    | Rotating a database password causes all service instances to reconnect simultaneously ó implement staggered rotation where instances reconnect over a drain window, not all at once                |
| Encryption overhead for secrets at rest      | Cloud secrets managers encrypt at rest by default, but custom secret storage (e.g., encrypted files) adds CPU overhead ó use managed secrets manager services that handle encryption transparently |

## Scope

This document defines the secrets management strategy for Vaeloom ó covering
...
secrets at rest (see [Encryption.md](./Encryption.md)), IAM for secrets manager
access (see [IAM.md](./IAM.md)).
...
> **Diagram:** Secrets lifecycle ó creation (encrypted at rest), startup loading
...
Vaeloom manages secrets (API keys, database credentials, JWT signing keys,
third-party tokens) using a tiered vault strategy. Environment-specific secrets
are stored in production-grade vaults (AWS Secrets Manager or HashiCorp Vault)
and injected into services at runtime. Local development and CI secrets are
managed through environment variables with strict `.env` governance.

This document defines the secret lifecycle, storage tiers, access control,
rotation policies, and emergency procedures for secret compromise. The primary
audience is DevOps engineers managing secret infrastructure and developers using
secrets in their services.

Within the Vaeloom platform, secrets fall into three tiers: Tier 1 (platform
credentials ó DB passwords, JWT keys, encryption keys), Tier 2 (third-party
tokens ó OpenAI API key, SendGrid API key, OAuth client secrets), and Tier 3
(CI/CD secrets ó Docker registry tokens, deployment SSH keys). Each tier has
...
never hardcoded, never committed to source control, never logged, and always
rotated on a defined schedule or on suspicion of compromise. Access to secrets
is audited, time-limited, and role-scoped.
...
- Service mesh secrets (Istio mTLS certificates ó covered in future service mesh
  deployment)
...
> **Diagram:** Secret lifecycle ó services request secrets from vault with
