...
```mermaid
flowchart LR
 subgraph Queues["Task Queues -- 8 isolated (no starvation)"]
 Q1["vaeloom-ingest-q<br/>20"]
 Q2["vaeloom-agent-q<br/>8<br/>LangGraph seam"]
 Q3["vaeloom-connectors-q<br/>6"]
 Q4["vaeloom-approvals-q<br/>20"]
 Q5["vaeloom-events-q<br/>8"]
 Q6["vaeloom-schedules-q<br/>4"]
 Q7["vaeloom-documents-q<br/>2"]
 Q8["vaeloom-memory-q<br/>2"]
 end
 subgraph Workflows["Workflows -- deterministic IDs REJECT_DUPLICATE"]
 W1["Ingest<br/>ingest:{ws}:{hash}:{doc}"]
 W2["DurableAgent<br/>durable_run:{ws}:{user}:{req}"]
 W3["Connector<br/>connector_sync:{ws}:{id}:{token}"]
 W4["Approval<br/>approval:{ws}:{id}"]
 end
 Q1 -.-> W1
 Q2 -.-> W2
 Q3 -.-> W3
 ...
 style Q2 fill:#1e1b4b,stroke:#a78bfa,color:#fff
 style W2 fill:#1e1b4b,stroke:#a78bfa,color:#fff
```

## Task Queues

> **As-built note:** 8 queues are declared, but only
> ingest/agent/approvals/connectors/events are driven by starters in
> `routers/temporal.py` (ingest, connector-sync, durable-agent + cancel/signal).
> No ScheduleDispatcher, DocumentCompile, or MemoryConsolidation workflow
> exists; `check_quota` activity exists in code but was missing from the table —
> added below. `HelloWorkflow` is smoke-test only.
...
```mermaid
sequenceDiagram
 participant WF as Workflow<br/>thin shell
 participant ACT as Activity<br/>timeout + retry
 participant DB as Domain<br/>idempotent guard
 WF->>ACT: execute_activity<br/>with RetryPolicy + heartbeat
 ACT->>DB: SELECT ... WHERE workspace_id
 DB-->>ACT: validate idempotency
 ACT->>ACT: heartbeat 15s/30s<br/>is_cancelled?
 ACT-->>WF: result completed / failed / cancelled
 WF-->>WF: patch versioned<br/>REJECT_DUPLICATE
```
...
| `check_quota`             | 5s            | 1×                    | per-workspace quota gate (T-007)                                                     |
