...
| F-06    | real pgvector RAG not proven                                    | L1 PROVEN: against running Postgres+pgvector, `Embedding.vector` is a real `vector(1536)` column and cosine distance orders vectors correctly (near=0.0001, far=1.0). Also fixed the Alembic migrations (0001/0002) that referenced the invalid `postgresql.VECTOR` — now use `pgvector.sqlalchemy.Vector` so `alembic upgrade head` creates real vector columns on Postgres                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | **CLOSED (L1)**                       | `infrastructure/vector_store.py`; `models/schema.py` (`Vector(1536)`); migrations `0001`/`0002` (`Vector(1536)`); live check `verify_f06_pgvector.py`                                         |
| ZT-01   | Temporal start fail-open                                        | L1 PROVEN: with a real Temporal server up and `temporal_enabled=True`, `get_temporal_client()` returns a live `Client` (not `None`); when enabled+down it raises `TemporalUnavailableError`→503 (unit-verified). Fail-closed confirmed both branches.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | **CLOSED (L1)**                       | `temporal/client.py`; `main.py` handler; live check `verify_zt01_live.py`; `tests/test_temporal_client_fail_closed.py`                                                                        |
...
| F-01    | durable LangGraph not default                                   | L1 PROVEN: started a real `DurableAgentRunWorkflow` on live Temporal (`vaeloom-agent-q`) with `LANGGRAPH_ENABLED=true,percent=100`; the activity ran the LangGraph path and returned `status=completed` with `metadata.graph_version="v1"`. Durable execution + LangGraph topology confirmed on real Temporal. (`temporal_enabled`/`langgraph_enabled` remain `False` by default — safe default)                                                                                                                                                                                                                                                                                                                                                                                                                                                 | **CLOSED (L1)**                       | live check `verify_f01_durable.py`; `temporal/workflows.py`; `temporal/activities.py::durable_agent_run`; worker fix `temporal/worker.py` (interceptors `[]` not `None`)                      |
| F-02    | no `Send` fan-out                                               | L1 CLOSED 2026-09-06: native `Send` fan-out implemented and proven. `route_fanout` fans out the first parallel DAG layer into one `Send(fanout_worker, …)` per agent (clamped to §14 bound 8, fail-closed to single agent); `fanout_worker` runs one agent as read-only analysis and returns ONLY a `branch_results` entry (never raw agent state, so branches cannot clobber shared keys); `branch_results` uses a direct (non-Optional) `Annotated[..., add]` reducer; `fan_in` aggregates and continues to `tool_decision`. Unit: 9/9 pass (bound, empty, failure isolation, reducer wiring, fan-in merge). L1: durable run completed on `vaeloom-agent-q` with worker log `fan-out firing: 2 parallel branches (scheduler,gmail)`, zero branch failures. v1 fans out the first parallel layer only (later sequential layers not re-entered). | **CLOSED (L1)**                       | `graph/__init__.py` (`route_fanout`), `graph/nodes.py` (`fanout_worker_node`, `fan_in_node`), `graph/state.py` (`branch_results` reducer); `tests/test_graph_send_fanout.py` (9 pass)         |
...
| F-10    | real LLM reasoning                                              | Still BLOCKED on creds, re-verified 2026-09-06 on current HEAD: `DurableAgentRunWorkflow` on `vaeloom-agent-q` with `LANGGRAPH_ENABLED=true` and NO key returns `completed` (graceful degradation, mock path). Worker log grepped for key-like leakage: 0 lines. No live key was used or committed; real LLM token flow stays unverified pending a user-supplied credential (env only, never committed).                                                                                                                                                                                                                                                                                                                                                                                                                                         | **OPEN / BLOCKED (L1 — needs creds)** | `services/llm_service.py`; `temporal/activities.py::durable_agent_run`; live check `verify_f10_mock.py`                                                                                       |
...
| F-12    | distributed scrape quota                                        | L1 PROVEN: with a real Redis at `localhost:6379`, `_get_quota_backend()` returns `_RedisScrapeQuota` and enforces the shared per-workspace quota in order (True,True,True,False,False). (`test_scrape_quota.py` exercises the same backend with a fakeredis client at L3.)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | **CLOSED (L1)**                       | `tools/executor.py`; live check `verify_f12_redis.py`; `tests/test_scrape_quota.py`                                                                                                           |
| F-13    | tracing exporter logging-only                                   | L1 CLOSED 2026-09-06: end-to-end delivery proven both transports — OTLP/HTTP span `f13_http_span_v2` and OTLP/gRPC span `f13_grpc_span` (via the app's exact `OTLPSpanExporter()` no-arg construction, env-configured endpoint) both observed in collector debug-exporter logs. Prior non-observance root-caused to TWO harness artifacts, not app code: (1) `OTEL_SDK_DISABLED=true` (set for test runs) silently disables SDK span recording, so all earlier emits were no-ops; (2) a `debug` exporter added to the traces pipeline without an `exporters:` entry crash-looped the collector. No app change needed; temp collector config fully reverted.                                                                                                                                                                                      | **CLOSED (L1)**                       | `infrastructure/opentelemetry.py:29`; live checks `verify_f13_http.py` / `verify_f13_grpc.py`                                                                                                 |
...
- No PostgreSQL/pgvector instance → F-06 L1. **RESOLVED 2026-08-30**: Docker
  `postgres` (pgvector/pgvector:pg16) stood up; F-06 L1 proven.
- No LLM/connector credentials → F-10/F-08 L1. F-10 **still BLOCKED** (no real
  `LLM_API_KEY`); F-08 L3 already closed (connector state, no live creds
  needed).
- No Temporal/worker/Redis running → ZT-01/J7/chaos/perf L1. **RESOLVED
  2026-08-30**: Temporal (+worker), Redis, otelcol stood up via
  `docker compose --profile temporal --profile monitoring`; ZT-01/F-01/F-12/F-13
  L1 proven.
- F-04 needed Alembic migration → shipped (0025) and L3-verified earlier.

**FINAL GATE** now **PARTIALLY OPEN** for the durable path: F-03, ZT-02, F-04,
F-11, F-08, F-05, F-12, F-13 are code-closed (L3) and F-06, ZT-01, F-01, F-12,
F-13 are additionally L1-promoted against real infra. Remaining honest gaps:
**F-10** (real LLM, needs creds), **F-02** (native langgraph `Send` fan-out —
code gap, not infra), **F-07** (context/RAG unification, not mapped). L1 chaos
(J7 worker-SIGKILL) and perf gates were not executed.

---

## Discovered blockers during L1 promotion (2026-08-30)

1. **Alembic migrations broken for real Postgres (RLS policy).**
   `alembic upgrade head` fails at migration `0010` (RLS): a policy on
   `workspaces` references `workspace_id`, which does not exist as a column on
   `workspaces` (the workspace's own key is `id`). This blocks a clean schema
   build on Postgres and is orthogonal to the 14 findings — flagged for a
   follow-up, not fixed here (the L1 pgvector proof used
   `Base.metadata.create_all`, the same path the dev/test lifespan uses).
   **Severity: deploy-blocking for Postgres.**
2. **Temporal worker crashed on `interceptors=None`.** `temporal/worker.py`
   passed `interceptors=interceptors or None`; current `temporalio` rejects
   `None` (needs `[]`). Fixed (`interceptors or []`) — required for the worker
   (and thus F-01 L1) to start at all.
3. **Migration `0001`/`0002` used invalid `postgresql.VECTOR`.** Fixed to
   `pgvector.sqlalchemy.Vector(1536)` (part of F-06 closure).

## Discovered blockers during finish pass (2026-09-06)

4. **`apps/api/.env` wins over process env
   (`load_dotenv(..., override=True)`).** `config.py` loads the local `.env`
   with override, so `TEMPORAL_ENABLED=true` (and friends) exported in the shell
   are silently stomped at import. Local L1 runs required a temporary endpoint
   flip of the gitignored `.env`, restored byte-identical afterwards
   (checksum-verified). The `.env` holds live cloud credentials
   (Supabase/Upstash/provider key) — local-only, never committed, never used by
   these runs. Recommend documenting the override behavior in the dev runbook so
   the next L1 session does not lose an hour to it.
5. **langgraph ignores reducers wrapped in `Optional`.** `branch_results` first
   shipped as `Annotated[..., add] | None`; parallel writes then failed with
   `INVALID_CONCURRENT_GRAPH_UPDATE`. Fix: keep the channel directly Annotated
   (absence is still allowed — the TypedDict is `total=False`). Noted in a code
   comment at the declaration.
6. **Stale workers serve stale graphs.** A worker started before a code edit
   keeps the old topology (the F-02 L1 first attempt ran the pre-`Send` graph).
   Always restart source-run workers after touching `graph/`; verify via a log
   marker (`fan-out firing`) rather than assuming the fresh code is live.
...
- F-10 (real LLM) still BLOCKED on credentials — not fabricated.
- F-02 (native `Send` fan-out) is a code-architecture gap, not an infra gate.
- No mock was promoted to L1; live spans for F-13 were only partially observed
  (Docker Desktop Windows gRPC proxy limitation), and that is stated honestly.

Infra (Postgres+pgvector, Temporal, Redis, otelcol) was stood up and used to
honestly promote F-06, ZT-01, F-01, F-12, F-13 to L1. No security control was
weakened.
