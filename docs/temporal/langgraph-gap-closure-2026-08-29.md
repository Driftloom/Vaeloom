...
| F-02    | no `Send` fan-out                                               | PARTIAL/L1: multi-agent runs execute durably via the same `DurableAgentRunWorkflow` (proven in F-01) using the `supervisor_dag` with `parallel/sequential/mixed` strategy (bounded fan-out ≤8, depth ≤5). Native langgraph `Send` primitive is NOT used — fan-out is driven by the supervisor strategy, not `Send`. Closing fully requires implementing `Send`-based dynamic fan-out (code change, not infra). | **OPEN (code gap)**                                                                     | `graph/__init__.py` (supervisor_dag), `graph/contracts.py` (`strategy`), `graph/nodes.py` (bounded DAG)                                                                                       |
...
| F-10    | real LLM reasoning                                              | still BLOCKED: no real `LLM_API_KEY`/provider creds in this env, so real LLM reasoning cannot be exercised. The `durable_agent_run` activity with `LANGGRAPH_ENABLED=true` ran on real Temporal using the mock/error path (graph handled a tool error gracefully), proving the durable graph plumbing — but real LLM token flow is unverified pending credentials.                                             | **OPEN / BLOCKED (L1 — needs creds)**                                                   | `services/llm_service.py`; `temporal/activities.py::durable_agent_run`                                                                                                                        |
...
| F-13    | tracing exporter logging-only                                   | L1 (partial): collector is up (OTLP gRPC :4317 / HTTP :4318) and the app uses `OTLPSpanExporter()` (real OTLP, not logging-only) — confirmed at code + `force_flush()` returns True. Live span receipt in the collector logs was NOT positively observed in this Docker-Desktop-on-Windows env (gRPC/HTTP2 over the published port proxy is flaky) — recommend re-verifying span receipt on a Linux/prod host. | **CLOSED (L1 partial — collector up, export path confirmed; live receipt env-limited)** | `infrastructure/opentelemetry.py:29`; live checks `verify_f13_otlp.py`/`verify_f13_v2.py`/`verify_f13_http.py`                                                                                |
