...
- ReAct loop in loop.py
- Supervisor mode for multi-agent workflows
- Budget checking (_check_spend_and_quota)
- Approval flow (lookup_approval, consume_approval_for_action) with payload hash
  verification
- RAG context assembly before planning
- Agent card registry for static config
- Loop safety: oscillation detection, no-progress detection, budget limits
