...
Do **not** assume `ENT-P19` passed because a document says “complete.” Re-audit its actual artifacts and evidence before executing this phase.
...
On `NO-GO`, switch to `AUDIT_COMPLETED_PHASE` then `REMEDIATE_FAILED_PHASE` for `ENT-P19`. Re-run its full gate and generate a new handoff before resuming this file.
...
## 1. Mission
Validate production behavior, outcomes, security, AI quality, integrity, SLOs and rollback criteria.
...
- **Phase-specific rule:** Validate per-tenant adoption, billing, data/AI quality, SLOs, regions, plugins and evidence.
...
6. Validate per-tenant adoption, billing, data/AI quality, SLOs, regions, plugins and evidence.
...
- Apply phase rule: Validate per-tenant adoption, billing, data/AI quality, SLOs, regions, plugins and evidence.
