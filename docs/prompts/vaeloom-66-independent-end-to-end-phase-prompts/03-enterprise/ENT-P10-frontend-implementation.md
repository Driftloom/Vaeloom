...
Do **not** assume `ENT-P09` passed because a document says “complete.” Re-audit its actual artifacts and evidence before executing this phase.
...
- `DEL-ENT-P09-05` — WCAG/usability evidence plan; versioned, owned, reviewed and linked.
...
On `NO-GO`, switch to `AUDIT_COMPLETED_PHASE` then `REMEDIATE_FAILED_PHASE` for `ENT-P09`. Re-run its full gate and generate a new handoff before resuming this file.
...
## 1. Mission
Implement the approved frontend with typed contracts, accessibility, security, performance, tests and rollback-safe delivery.
...
- **Phase-specific rule:** Use policy-derived UI capability, localization, accessibility and admin audit.
...
6. Use policy-derived UI capability, localization, accessibility and admin audit.
...
- Apply phase rule: Use policy-derived UI capability, localization, accessibility and admin audit.
