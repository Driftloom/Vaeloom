...
Do **not** assume `ENT-P14` passed because a document says “complete.” Re-audit its actual artifacts and evidence before executing this phase.
...
On `NO-GO`, switch to `AUDIT_COMPLETED_PHASE` then `REMEDIATE_FAILED_PHASE` for `ENT-P14`. Re-run its full gate and generate a new handoff before resuming this file.
...
- **Phase-specific rule:** Validate noisy-neighbor, cell/control-plane failure, region loss, quotas, batch and cost.
...
6. Validate noisy-neighbor, cell/control-plane failure, region loss, quotas, batch and cost.
...
- Apply phase rule: Validate noisy-neighbor, cell/control-plane failure, region loss, quotas, batch and cost.
