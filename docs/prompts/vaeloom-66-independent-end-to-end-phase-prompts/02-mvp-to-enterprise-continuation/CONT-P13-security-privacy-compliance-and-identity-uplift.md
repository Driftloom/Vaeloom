...
Do **not** assume `CONT-P12` passed because a document says “complete.” Re-audit
...
`CONT-P12`. Re-run its full gate and generate a new handoff before resuming this
file.
...
> NOT_EXECUTED **Phase type:** `SECURITY` **Phase ID:** `CONT-P13`
...
Execute a controlled migration phase that will implement and independently
verify security, privacy, identity, consent, data rights, AI governance and
compliance mapping. Preserve backward compatibility, provenance, user rights and
rollback.
...
- **Out of scope:** Big-bang rewrite, silent permission expansion, unverified
  ...
  evidence.
- **Phase-specific rule:** Certify isolation, identity/SCIM, consent migration,
  key rotation, DPIA and incidents.
...
5. Create incident/breach/evidence/vulnerability/exception governance and
   independent testing.
6. Certify isolation, identity/SCIM, consent migration, key rotation, DPIA and
   incidents.
...
- Preserve approved architecture: Strangler/expand-contract path with
  ...
  explicit retirement.
- Apply phase rule: Certify isolation, identity/SCIM, consent migration, key
  rotation, DPIA and incidents.
...
Prepare for **Migration Testing, Reconciliation, and Certification**: approved
