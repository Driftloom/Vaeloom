...
Do **not** assume `CONT-P07` passed because a document says “complete.” Re-audit
...
- **Current phase:**
  `CONT-P08 — API, Event, Connector, and Contract Compatibility`
...
`CONT-P07`. Re-run its full gate and generate a new handoff before resuming this
file.
...
> NOT_EXECUTED **Phase type:** `API_DESIGN` **Phase ID:** `CONT-P08`
...
Execute a controlled migration phase that will define versioned API, event,
webhook, SDK, MCP, identity, authorization, error, idempotency and async-job
contracts. Preserve backward compatibility, provenance, user rights and
rollback.
...
- **Out of scope:** Big-bang rewrite, silent permission expansion, unverified
  ...
  evidence.
- **Phase-specific rule:** Use additive contracts, tolerant readers, schema
  registry, consumer inventory and shadow traffic.
...
7. Use additive contracts, tolerant readers, schema registry, consumer inventory
   and shadow traffic.
...
- Preserve approved architecture: Strangler/expand-contract path with
  ...
  explicit retirement.
- Apply phase rule: Use additive contracts, tolerant readers, schema registry,
  consumer inventory and shadow traffic.
...
- `DEL-CONT-P08-02` — event/webhook/job schemas; versioned, owned, reviewed and
  linked.
...
- `DEL-CONT-P08-05` — compatibility/deprecation policy; versioned, owned,
  reviewed and linked.
- Updated risk, decision, assumption, evidence, traceability and change
  registers.
- Gate report and next-phase handoff.
...
Prepare for **UX, Consent, Admin, and Change-Experience Migration**: approved
