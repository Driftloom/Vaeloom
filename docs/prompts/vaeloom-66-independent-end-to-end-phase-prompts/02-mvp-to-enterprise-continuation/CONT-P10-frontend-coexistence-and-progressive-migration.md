...
Do **not** assume `CONT-P09` passed because a document says “complete.” Re-audit its actual artifacts and evidence before executing this phase.

### Predecessor identity
- **Previous phase:** `CONT-P09 — UX, Consent, Admin, and Change-Experience Migration`
...
On `NO-GO`, switch to `AUDIT_COMPLETED_PHASE` then `REMEDIATE_FAILED_PHASE` for `CONT-P09`. Re-run its full gate and generate a new handoff before resuming this file.
...
- Enforce frontend performance and accessibility budgets in CI while retaining manual assistive-technology validation.
- Design resilient client state for retries, duplicate submissions, stale approvals, partial async jobs and reconnects.
- Prevent sensitive context from leaking into client logs, analytics, caches or error reporting.
- Instrument user-visible success and failure states without dark patterns or inaccessible motion.
...
> **Phase type:** `FRONTEND_IMPLEMENTATION`
> **Phase ID:** `CONT-P10`
...
## 1. Mission
Execute a controlled migration phase that will implement the approved frontend with typed contracts, accessibility, security, performance, tests and rollback-safe delivery. Preserve backward compatibility, provenance, user rights and rollback.
...
Frontend Engineer; UX Lead; Accessibility Specialist; API Engineer; Security Engineer; QA Automation Engineer; SRE.
...
- **Phase-specific rule:** Use route/component flags and adapters; avoid unrelated frontend forks.
...
## 11. Workstreams
1. **WS-10.1: Application shell/routing.** Assign owner, inputs, dependencies, acceptance, tests, evidence, risks and handoff.
2. **WS-10.2: Typed data/state.** Assign owner, inputs, dependencies, acceptance, tests, evidence, risks and handoff.
3. **WS-10.3: Feature workflows.** Assign owner, inputs, dependencies, acceptance, tests, evidence, risks and handoff.
4. **WS-10.4: Accessibility/performance/security.** Assign owner, inputs, dependencies, acceptance, tests, evidence, risks and handoff.
5. **WS-10.5: Tests/delivery.** Assign owner, inputs, dependencies, acceptance, tests, evidence, risks and handoff.

## 12. Detailed Tasks and Subtasks
1. Set up typed app architecture, routes, boundaries, state/query and secure sessions.
2. Generate clients from contracts and fail CI on drift.
3. Implement all workflows/states without false optimistic success.
4. Use policy-derived capability display while server authorization remains authoritative.
...
- Apply phase rule: Use route/component flags and adapters; avoid unrelated frontend forks.
...
Run type/lint/unit/component/contract/E2E/accessibility/security/performance tests.
