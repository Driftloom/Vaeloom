...
Do **not** assume `CONT-P05` passed because a document says “complete.” Re-audit
...
`CONT-P05`. Re-run its full gate and generate a new handoff before resuming this
file.
...
> NOT_EXECUTED **Phase type:** `TECHNOLOGY_SELECTION` **Phase ID:** `CONT-P06`
...
Execute a controlled migration phase that will select and pin technologies using
compatibility, security, performance, cost, support and exit evidence. Preserve
...
- **Out of scope:** Big-bang rewrite, silent permission expansion, unverified
  ...
  evidence.
- **Phase-specific rule:** Each technology change needs benchmark,
  compatibility, training, rollback and exit.
...
6. Each technology change needs benchmark, compatibility, training, rollback and
   exit.
...
- Preserve approved architecture: Strangler/expand-contract path with
  ...
  explicit retirement.
- Apply phase rule: Each technology change needs benchmark, compatibility,
  training, rollback and exit.
...
Prepare for **Tenant, Data, Memory, and Knowledge Migration**: approved
