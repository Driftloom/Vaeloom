...
Do **not** assume `CONT-P10` passed because a document says “complete.” Re-audit
its actual artifacts and evidence before executing this phase.

### Predecessor identity

- **Previous phase:**
  `CONT-P10 — Frontend Coexistence and Progressive Migration`
...
`CONT-P10`. Re-run its full gate and generate a new handoff before resuming this
file.
...
> NOT_EXECUTED **Phase type:** `BACKEND_IMPLEMENTATION` **Phase ID:** `CONT-P11`
...
Execute a controlled migration phase that will implement core services with
authorization, isolation, async processing, idempotency, integrity, audit and
operations. Preserve backward compatibility, provenance, user rights and
rollback.
...
- **Out of scope:** Big-bang rewrite, silent permission expansion, unverified
  ...
  evidence.
- **Phase-specific rule:** Extract services only for measured need; prove
  outbox/order/idempotency.
...
- Preserve approved architecture: Strangler/expand-contract path with
  ...
  explicit retirement.
- Apply phase rule: Extract services only for measured need; prove
  outbox/order/idempotency.
...
Prepare for **Agent, Model, Retrieval, and Memory-Taxonomy Migration**: approved
