...
Do **not** assume `CONT-P15` passed because a document says “complete.” Re-audit
...
`CONT-P15`. Re-run its full gate and generate a new handoff before resuming this
file.
...
> NOT_EXECUTED **Phase type:** `DEVOPS` **Phase ID:** `CONT-P16`
...
request:
  ...
  current_phase_name:
    Platform Migration, Infrastructure, and Delivery Automation
execution_rules:
...
Execute a controlled migration phase that will create reproducible secure
environments and delivery pipelines with IaC, SBOM, provenance, signing,
rollback and recovery. Preserve backward compatibility, provenance, user rights
and rollback.
...
- **Out of scope:** Big-bang rewrite, silent permission expansion, unverified
  ...
  evidence.
- **Phase-specific rule:** Use immutable artifacts, per-tenant flags and
  automatic pause on divergence.
...
- Preserve approved architecture: Strangler/expand-contract path with
  ...
  explicit retirement.
- Apply phase rule: Use immutable artifacts, per-tenant flags and automatic
  pause on divergence.
...
Prepare for **Dual-Run Observability and Migration Operations**: approved
