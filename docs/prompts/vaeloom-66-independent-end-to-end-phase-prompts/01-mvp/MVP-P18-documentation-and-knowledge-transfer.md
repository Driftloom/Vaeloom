...
Do **not** assume `MVP-P17` passed because a document says “complete.” Re-audit its actual artifacts and evidence before executing this phase.
...
On `NO-GO`, switch to `AUDIT_COMPLETED_PHASE` then `REMEDIATE_FAILED_PHASE` for `MVP-P17`. Re-run its full gate and generate a new handoff before resuming this file.
...
- **Phase-specific rule:** Resolve canonical/superseded docs and separate design from implementation status.
...
6. Resolve canonical/superseded docs and separate design from implementation status.
...
- Apply phase rule: Resolve canonical/superseded docs and separate design from implementation status.
