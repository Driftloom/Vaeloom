...
Do **not** assume `MVP-P10` passed because a document says “complete.” Re-audit its actual artifacts and evidence before executing this phase.
...
- `DEL-MVP-P10-05` — performance/deploy notes; versioned, owned, reviewed and linked.
...
On `NO-GO`, switch to `AUDIT_COMPLETED_PHASE` then `REMEDIATE_FAILED_PHASE` for `MVP-P10`. Re-run its full gate and generate a new handoff before resuming this file.
...
- **Phase-specific rule:** Implement identity/isolation before files or agents; enforce scope everywhere.
...
6. Implement identity/isolation before files or agents; enforce scope everywhere.
...
- Apply phase rule: Implement identity/isolation before files or agents; enforce scope everywhere.
