...
Do **not** assume `MVP-P14` passed because a document says “complete.” Re-audit its actual artifacts and evidence before executing this phase.
...
On `NO-GO`, switch to `AUDIT_COMPLETED_PHASE` then `REMEDIATE_FAILED_PHASE` for `MVP-P14`. Re-run its full gate and generate a new handoff before resuming this file.
...
## 1. Mission
Validate capacity, latency, throughput, cost, availability, recoverability, degradation, scaling and DR through evidence.
...
- **Phase-specific rule:** Measure queue/model/retrieval/cost triggers before architecture split.
...
- Apply phase rule: Measure queue/model/retrieval/cost triggers before architecture split.
...
- `DEL-MVP-P15-02` — load/resilience results; versioned, owned, reviewed and linked.
