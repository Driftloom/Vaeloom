# Vaeloom MVP — Independent End-to-End Phase Prompt 10: Frontend Implementation
...
Do **not** assume `MVP-P09` passed because a document says “complete.” Re-audit
its actual artifacts and evidence before executing this phase.

### Predecessor identity

- **Previous phase:** `MVP-P09 — UI/UX and Design System`
- **Current phase:** `MVP-P10 — Frontend Implementation`
...
- `DEL-MVP-P09-05` — WCAG/usability evidence plan; versioned, owned, reviewed
  and linked.
...
`MVP-P09`. Re-run its full gate and generate a new handoff before resuming this
file.
...
| -------------- | ----------------------------------- | ----------------- | ----------------- | ------------ | -------------- | -------------- | ------------------ |
| PA-MVP-P10-001 | TO_BE_VERIFIED                      | TO_BE_VERIFIED    | TO_BE_VERIFIED    | NOT_EXECUTED | TO_BE_VERIFIED | TO_BE_VERIFIED | TO_BE_VERIFIED     |
...
# Enterprise Execution Prompt — MVP Phase 10: Frontend Implementation
...
> **Phase type:** `FRONTEND_IMPLEMENTATION` **Phase ID:** `MVP-P10`

```yaml
request:
  mode: GENERATE_AND_EXECUTE_PHASE
  current_phase_id: MVP-P10
...
- **Phase-specific rule:** Ship web first; desktop/VS Code are separately scoped
...
| ----------- | ---------------- | ----------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------- |
| MVP-P10-R01 | Scope            | Implement the approved frontend with typed contracts, accessibility, security, performance, tests and rollback-safe delivery. | Approved objective/scope/owners/authority                 |
| MVP-P10-R02 | Evidence         | Every material claim links to authoritative source or reproducible evidence.                                                  | No unsupported completion/security/compliance/scale claim |
| MVP-P10-R03 | Security/Privacy | Applicable security, privacy, abuse, rights and AI risks are designed, tested and owned.                                      | No unresolved critical finding or expired waiver          |
| MVP-P10-R04 | Quality          | Validation covers normal, negative, boundary, failure and recovery.                                                           | Critical tests pass in representative environment         |
| MVP-P10-R05 | Operations       | Ownership, telemetry, support, rollback and lifecycle are included.                                                           | Runbook/rollback/telemetry evidence exists                |
| MVP-P10-R06 | Data/AI          | Data lineage, scope, quality, retention and AI lineage are explicit.                                                          | Source/version/owner/lifecycle reconstructable            |
| MVP-P10-R07 | Traceability     | Requirements map to design, artifacts, tests, evidence, risks and handoff.                                                    | No unexplained critical gap                               |
| MVP-P10-R08 | Gate             | Progression is blocked until DoD and weighted gate pass.                                                                      | Score ≥95 and zero mandatory blockers                     |
...
6. Ship web first; desktop/VS Code are separately scoped clients and releases.
...
- Apply phase rule: Ship web first; desktop/VS Code are separately scoped
  clients and releases.
...
| --------------- | ----------------------- | ----------- | ------------------------ | -------------- | ------------ | -------------- | -------------- |
| EVD-MVP-P10-001 | Replace with real claim | MVP-P10-R01 | file/log/report/approval | TO_BE_VERIFIED | NOT_EXECUTED | TO_BE_VERIFIED | TO_BE_VERIFIED |
...
| --------------- | ------------------------------------------- | -------- | ------------------ | --------------------------------------- | ----------------------------- | ------ |
| RISK-MVP-P10-01 | Docs mistaken for runtime completion        | Critical | False readiness    | Require runtime evidence/status labels  | Phase owner                   | OPEN   |
| RISK-MVP-P10-02 | Scope/permission/data/compatibility assumed | High     | Leak/loss/rework   | Block or reversible validated decision  | Product/Architecture/Security | OPEN   |
| RISK-MVP-P10-03 | External API/model/standard changes         | High     | Regression         | Pin versions, tests, owner, kill switch | Integration/AI                | OPEN   |
| RISK-MVP-P10-04 | Evidence incomplete                         | High     | Untrustworthy gate | Immutable reports and baseline          | QA/Release                    | OPEN   |
| RISK-MVP-P10-05 | MVP scope expansion                         | High     | Delay/complexity   | Strict scope gate                       | Product                       | OPEN   |
