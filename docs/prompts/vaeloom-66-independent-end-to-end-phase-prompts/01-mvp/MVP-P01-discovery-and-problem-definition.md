...
Do **not** assume `MVP-P00` passed because a document says “complete.” Re-audit its actual artifacts and evidence before executing this phase.
...
On `NO-GO`, switch to `AUDIT_COMPLETED_PHASE` then `REMEDIATE_FAILED_PHASE` for `MVP-P00`. Re-run its full gate and generate a new handoff before resuming this file.
