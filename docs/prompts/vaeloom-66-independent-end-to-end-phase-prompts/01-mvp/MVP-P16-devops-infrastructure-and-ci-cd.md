...
Do **not** assume `MVP-P15` passed because a document says “complete.” Re-audit its actual artifacts and evidence before executing this phase.
...
On `NO-GO`, switch to `AUDIT_COMPLETED_PHASE` then `REMEDIATE_FAILED_PHASE` for `MVP-P15`. Re-run its full gate and generate a new handoff before resuming this file.
...
- **Phase-specific rule:** PaaS-first still requires IaC, secrets, backups, signed images, staging and rollback.
...
6. PaaS-first still requires IaC, secrets, backups, signed images, staging and rollback.
...
- Apply phase rule: PaaS-first still requires IaC, secrets, backups, signed images, staging and rollback.
