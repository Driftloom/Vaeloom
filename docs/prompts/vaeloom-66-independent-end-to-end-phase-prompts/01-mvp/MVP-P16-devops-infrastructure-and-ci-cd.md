# Vaeloom MVP — Independent End-to-End Phase Prompt 16: DevOps, Infrastructure, and CI/CD
...
Do **not** assume `MVP-P15` passed because a document says “complete.” Re-audit
...
- **Current phase:** `MVP-P16 — DevOps, Infrastructure, and CI/CD`
...
`MVP-P15`. Re-run its full gate and generate a new handoff before resuming this
file.
...
| -------------- | ----------------------------------- | ----------------- | ----------------- | ------------ | -------------- | -------------- | ------------------ |
| PA-MVP-P16-001 | TO_BE_VERIFIED                      | TO_BE_VERIFIED    | TO_BE_VERIFIED    | NOT_EXECUTED | TO_BE_VERIFIED | TO_BE_VERIFIED | TO_BE_VERIFIED     |
...
- Use immutable infrastructure, policy-as-code, signed artifacts, SBOM, SLSA
  provenance and environment promotion evidence.
- Separate build, deploy and approval authority; protect secrets and production
  credentials from CI workloads.
- Automate canary/blue-green rollout, migration prechecks, rollback and
  disaster-recovery drills.
- Keep infrastructure portable enough to avoid a forced rewrite when regions or
...
# Enterprise Execution Prompt — MVP Phase 16: DevOps, Infrastructure, and CI/CD
...
> **Phase type:** `DEVOPS` **Phase ID:** `MVP-P16`

```yaml
request:
  mode: GENERATE_AND_EXECUTE_PHASE
  current_phase_id: MVP-P16
...
Create reproducible secure environments and delivery pipelines with IaC, SBOM,
provenance, signing, rollback and recovery.
...
Platform Engineer; DevOps Engineer; Cloud Architect; Security Engineer; SRE;
Database Engineer; Release Manager.
...
- **Phase-specific rule:** PaaS-first still requires IaC, secrets, backups,
  signed images, staging and rollback.
...
| ----------- | ---------------- | -------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------- |
| MVP-P16-R01 | Scope            | Create reproducible secure environments and delivery pipelines with IaC, SBOM, provenance, signing, rollback and recovery. | Approved objective/scope/owners/authority                 |
| MVP-P16-R02 | Evidence         | Every material claim links to authoritative source or reproducible evidence.                                               | No unsupported completion/security/compliance/scale claim |
| MVP-P16-R03 | Security/Privacy | Applicable security, privacy, abuse, rights and AI risks are designed, tested and owned.                                   | No unresolved critical finding or expired waiver          |
| MVP-P16-R04 | Quality          | Validation covers normal, negative, boundary, failure and recovery.                                                        | Critical tests pass in representative environment         |
| MVP-P16-R05 | Operations       | Ownership, telemetry, support, rollback and lifecycle are included.                                                        | Runbook/rollback/telemetry evidence exists                |
| MVP-P16-R06 | Data/AI          | Data lineage, scope, quality, retention and AI lineage are explicit.                                                       | Source/version/owner/lifecycle reconstructable            |
| MVP-P16-R07 | Traceability     | Requirements map to design, artifacts, tests, evidence, risks and handoff.                                                 | No unexplained critical gap                               |
| MVP-P16-R08 | Gate             | Progression is blocked until DoD and weighted gate pass.                                                                   | Score ≥95 and zero mandatory blockers                     |
...
1. **WS-16.1: IaC/environments.** Assign owner, inputs, dependencies,
   acceptance, tests, evidence, risks and handoff.
2. **WS-16.2: Secure CI/supply chain.** Assign owner, inputs, dependencies,
   acceptance, tests, evidence, risks and handoff.
3. **WS-16.3: Progressive deployment/rollback.** Assign owner, inputs,
   dependencies, acceptance, tests, evidence, risks and handoff.
4. **WS-16.4: Migration/backup/DR automation.** Assign owner, inputs,
   dependencies, acceptance, tests, evidence, risks and handoff.
5. **WS-16.5: Policy/evidence.** Assign owner, inputs, dependencies, acceptance,
   tests, evidence, risks and handoff.

## 12. Detailed Tasks and Subtasks

1. Define accounts/networks/IAM/secrets/compute/data/storage/observability as
   IaC.
2. Build CI for
   lint/type/test/contracts/security/accessibility/AI/IaC/images/docs.
3. Generate SBOM/provenance, sign artifacts and verify before deploy.
4. Use immutable promotion, protected environments, progressive rollout and
   rollback.
5. Automate expand-contract, backup/restore, rotation and DR with evidence.
6. PaaS-first still requires IaC, secrets, backups, signed images, staging and
   rollback.
...
- Apply phase rule: PaaS-first still requires IaC, secrets, backups, signed
  images, staging and rollback.
...
Validate IaC, policy, supply chain, signatures,
deploy/rollback/migrations/DR/secrets.
...
| --------------- | ----------------------- | ----------- | ------------------------ | -------------- | ------------ | -------------- | -------------- |
| EVD-MVP-P16-001 | Replace with real claim | MVP-P16-R01 | file/log/report/approval | TO_BE_VERIFIED | NOT_EXECUTED | TO_BE_VERIFIED | TO_BE_VERIFIED |
...
| --------------- | ------------------------------------------- | -------- | ------------------ | --------------------------------------- | ----------------------------- | ------ |
| RISK-MVP-P16-01 | Docs mistaken for runtime completion        | Critical | False readiness    | Require runtime evidence/status labels  | Phase owner                   | OPEN   |
| RISK-MVP-P16-02 | Scope/permission/data/compatibility assumed | High     | Leak/loss/rework   | Block or reversible validated decision  | Product/Architecture/Security | OPEN   |
| RISK-MVP-P16-03 | External API/model/standard changes         | High     | Regression         | Pin versions, tests, owner, kill switch | Integration/AI                | OPEN   |
| RISK-MVP-P16-04 | Evidence incomplete                         | High     | Untrustworthy gate | Immutable reports and baseline          | QA/Release                    | OPEN   |
| RISK-MVP-P16-05 | MVP scope expansion                         | High     | Delay/complexity   | Strict scope gate                       | Product                       | OPEN   |
...
Prepare for **Observability and Operations**: approved
