...
- `apps/backend/src/backend/schemas/memory_types.py` (22 memory types registry)
- `apps/backend/src/backend/agents/memory_agent/handler.py` (MemoryAgent scope:
  only reads/writes2 of22 types)
- `apps/backend/src/backend/models/schema.py:212` (Memory model with `type`
  string field, `graph_node_id`, `vector_id`)
- `apps/backend/src/backend/orchestrator/loop.py:83` (approval gate hardcoded
  OFF)
