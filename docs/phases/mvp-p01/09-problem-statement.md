...
> baseline `1def16d`. **Owner:** Product Manager / UX Researcher. **Evidence
> labels:** SOURCE_DERIVED (spec/repo), EXTERNAL_VERIFIED (research brief
> `07-research-brief-2026-08-07.md`), STAKEHOLDER_DECISION (user 2026-08-13),
> UNKNOWN (no live-user research available - never fabricated). **Prompt
> reference:** `MVP-P01-discovery-and-problem-definition.md` §1/§2/§3/
> §5/§9/§12/§16. **Phase rule:** validate the student/early-career wedge WITHOUT
...
- **Evidence basis:** SOURCE_DERIVED - `docs/01-vaeloom-mvp-spec.md` §2 (resumes
  ...
  truth); §7 (six memory types exist precisely because facts recur across
  features). EXTERNAL_VERIFIED - `07-research-brief-2026-08-07.md` §1 (45M
  enrolled, ~25% NEET, youth unemployment 9.9-14.9%, high application volume per
  seeker; §3 250+ resumes per opening). STAKEHOLDER_DECISION - India launch,
  18+, individual job seekers (2026-08-13).
...
- **Current journey (as-is):** user creates/edits resume manually -> searches
  ...
  applications/outcomes is maintained.
...
- **Current journey (as-is):** no product to point at yet - the trust risk is
  ...
  account-flagging harms (research brief §3).
...
Indian job seekers (18+) that (a) reuses profile/career/document memory so users
never re-enter data (PS-01), (b) keeps career documents current and
role-tailored against ATS reality (PS-02), (c) extracts deadlines from Gmail and
tracks applications (PS-03), and (d) stays suggest-mode-first with draft-only
Gmail and approved-integration-only submission (PS-04 trust). Resume
...
| S-02 | Consequential actions require immutable, payload-bound, expiring user approval plus idempotency; no action without it                                                   | Prompt §3                       | Approval contract verification P05/P13 (VB-03)                                      |
