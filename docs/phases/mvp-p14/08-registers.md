...
| EXC-P14-02 | WCAG 2.2 AA not re-measured (apps/web jest 37 but no axe)                             | A11y Spec | **PARTIALLY CLOSED 2026-08-22** `apps/web/src/__tests__/a11y.test.tsx` smoke shell + `testing/accessibility/axe-config.ts` WCAG22AA threshold 0/5/10/20 — `jest-axe` fallback structural checks pass; full `axe` scan deferred to P15 | A11y      | P15      | `pnpm --filter web test -- src/__tests__/a11y.test.tsx` | Claim AA without `axe`      |
| EXC-P14-03 | Perf p50/p95 not benched this phase                                                   | Perf Eng  | Rate limiter + circuit breaker verified via code; `sanitize_text` now wired in `tools/executor.py:15` (closes F-11)                                                                                                                   | Perf      | P15      | `wrk/k6`                                                | Claim scalable              |
| EXC-P14-04 | `testing/smoke/, security/, chaos/, fuzz/, visual-regression/` EMPTY per AGENTS.md:87 | QA        | **PARTIALLY CLOSED 2026-08-22** `testing/smoke/README.md` inventory 5 suites/12 cases + `apps/api/tests/smoke/test_health.py` 2 tests — unit+integ 152 files + smoke 12                                                               | QA        | Post-MVP | Inventory                                               | Claim full QA without smoke |
