...
| `IP_ALLOWLIST`           | `` (empty → no-op, middleware ALWAYS mounted)                                                        | `config.py:ip_allowlist`            | Always mounted `main.py:188` no-op when empty (was stale NOT MOUNTED fixed F-18); AGENT RLS 37/42 (was stale 4/36 fixed F-04, 0019 now fail-closed F-05) — intentional |
