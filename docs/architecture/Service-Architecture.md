...
> **Purpose:** Define the service architecture and inter-service communication
> for Vaeloom **Status:** ✅ Upgraded to enterprise quality **Canonical
> source:**
...
| Path                    | Protocol  | Notes                              |
| ----------------------- | --------- | ---------------------------------- |
| Web → Backend           | HTTP/REST | Standard API calls                 |
| Backend → External APIs | HTTPS     | Model API (Anthropic/OpenAI) calls |
...
- Define clear service boundaries and responsibilities for web, API, and AI
  service tiers
- Establish secure inter-service communication patterns with mTLS and service
  tokens
- Prevent direct service-to-service database access through well-defined API
  contracts
...
| ---------------------------------------------------------- | -------------------------------------------- |
| Two-service architecture (web, backend) responsibilities   | Individual function-level implementation     |
| Inter-service communication protocols (HTTP, WebSocket)    | Third-party API integration specifics        |
...
| --------- | ------------------------------------------------------------------------------ | -------- |
| SVC-FR-01 | Backend service must be the sole external gateway; all AI logic runs within it | P0       |
...
| ---------------- | ------------------------------------------------------------------------- | ------------------------------------------------- | ------------------------------------------------ |
| Backend Service  | Auth, CRUD, permission enforcement, agents, memory, RAG, event publishing | FastAPI, Python 3.12                              | Horizontal auto-scaling based on request latency |
...
1. Frontend sends authenticated HTTP request to the Backend Service, which
   validates the JWT and checks permissions against the Permission Engine
2. Backend Service determines if the request requires AI processing or is a
   standard CRUD operation against PostgreSQL
3. For AI operations, Backend Service routes internally to the agent
   orchestration layer
4. AI agents process the request, retrieve context from PostgreSQL/Redis, call
   the external Model API, and return the result
5. Backend Service enriches the AI response with permission context, writes
   audit log entry, publishes relevant events to the bus, and returns the
