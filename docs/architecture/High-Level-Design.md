...
> **Purpose:** High-level architectural design for Vaeloom **Status:** ✅
...
graph TD
 ...
 User-.-> Desktop[Desktop Companion (Phase 2 Roadmap)]
 User-.-> VSCode[VS Code Extension (Phase 2 Roadmap)]
 Web--> API[Backend Service FastAPI]
 API--> PG[(PostgreSQL)]
...
| Decision                       | Rationale                                                            |
| ------------------------------ | -------------------------------------------------------------------- |
| Monolithic FastAPI backend     | Single deployable, simpler ops; all AI and CRUD logic in one service |
...
| Practice                                                                              | Rationale                                                                                                                                                                                     |
| ------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Enforce that all frontend traffic routes through the backend service                  | The frontend talks to the backend (FastAPI); all logic runs within the single backend service                                                                                                 |
...
| ---------------------------------------------------- | --------------------------------------------------------------------- |
| Two-service decomposition (web, backend)             | Detailed class-level design and implementation                        |
...
| Web Application UI & REST/SSE API                    | Desktop Companion & VS Code Extension (Phase 2 Post-MVP / Enterprise) |
...
| Backend Service | Authentication, rate limiting, request routing, agents, memory, RAG | FastAPI, Python 3.12       | Stateless horizontal scaling behind load balancer |
| PostgreSQL      | Relational data, graph entities, vector embeddings                  | PostgreSQL 16 + pgvector   | Vertical → read replicas → partitioning           |

## Data Flow

1. User initiates a request from the browser which hits the CDN edge and is
   forwarded to the Web Frontend
2. Web Frontend authenticates the session via the API Gateway, which validates
   tokens with the Auth Provider
3. For AI operations, the API Gateway forwards authorized requests to the AI
   Service via internal RPC with mTLS
4. AI Service retrieves relevant context from PostgreSQL (entities +
   embeddings), constructs agent prompts, and calls the external Model API
5. The response flows back through the AI Service → API Gateway → Web Frontend,
   with structured logging at every hop
