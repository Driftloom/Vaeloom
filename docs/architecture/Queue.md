...
> **Purpose:** Define the queue and async processing architecture for Vaeloom —
> how background jobs are enqueued, processed, retried, and monitored
> **Status:** ✅ Upgraded to enterprise quality **Owner:** DevOps Team **Last
> Updated:** 2026-07-12
...
Vaeloom relies on async processing for document ingestion, agent execution,
connector syncs, and scheduled tasks. Queues decouple request submission from
...
sequenceDiagram
 ...
 participant R as --„¸ Redis<br/>(BullMQ Backend)
 participant BS as ° BullMQ Scheduler<br/>(Delayed Jobs)
 participant W as ™¸ Worker
 participant M as "Š Monitoring
...
> **Diagram:** The sequence flows from left to right through three scenarios.
> **Top:** standard enqueue → pub/sub notification → worker claims job → success
> or retry → dead letter. **Middle:** delayed jobs scheduled via BullMQ's
> delayed sorted set with ZADD/ZPOPMIN. **Bottom:** real-time pub/sub events for
...
| ----------- | ---------------------------------------------------- | ------------------------------------------- | -------------------------------------- |
| MVP | Redis + BullMQ (installed but NO consumers deployed) | Simple to operate, already needed for cache | — |
...
Jobs that fail after max retries (3 attempts) go to a dead letter queue for
manual review:
...
- Guarantee at-least-once delivery for all critical jobs (ingestion, memory
 extraction)
...
- Keep dead letter queue volume under 10 jobs per day through robust error
 handling
- Enable independent scaling of worker pools per queue type without cross-queue
 contention
...
| ------------------- | ------------------------------------------------------ | -------------------------------------------------------- | -------------------------------------------------- |
| Job Producer | Enqueue jobs from API, cron, and agent services | Python BullMQ producer | Stateless, auto-scale with service |
...
| BullMQ Worker | Claim and process jobs with concurrency control | BullMQ worker library (installed, no consumers deployed) | Per-queue worker pool with horizontal auto-scaling |
...
1. Producer creates a job with payload, job ID, and options (priority, delay,
 TTL) and adds it to BullMQ
2. BullMQ stores the job as a Redis hash, adds the job ID to the waiting list,
 and publishes a job:available event
3. Idle worker receives the event via Redis pub/sub, claims the job via BLPOP,
 and moves it to active state
4. Worker executes the job handler — on success, job is ACKed and removed; on
 failure, retry count increments
5. After max retries (3), job is moved to the dead letter queue, and an alert is
 sent to engineering for manual review
