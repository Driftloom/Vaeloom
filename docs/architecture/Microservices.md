...
> **Purpose:** Define the microservices architecture for Vaeloom **Status:** ✅
...
graph TD
 ...
 subgraph Services["Service Decomposition (TARGET)"]
 direction TB
 ...
 subgraph Data_Ownership["Data Ownership"]
 direction TB
...
| ----------- | ----------------- | ---------------------------- |
| Synchronous | HTTP/REST | Frontend → Backend |
| Async | Event bus (Redis) | Agent action → Event publish |
...
- Decompose the platform into four independently deployable, independently
 scalable services
- Enforce data ownership boundaries where no service directly accesses another's
 database
- Provide clear communication patterns (sync RPC, async events, streaming) for
 each service interaction
- Support three deployment environments (dev Docker Compose, staging PaaS,
 production K8s)
- Ensure each service has its own health, readiness, and metrics endpoints for
 orchestration
