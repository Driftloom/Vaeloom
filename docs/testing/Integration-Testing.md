...
graph TD
 ...
 T3["Backend--> Redis<br/>Queue + cache<br/>Redis test instance"]
 T4["AI Logic--> Database<br/>Memory read/write<br/>PostgreSQL test instance"]
...
Integration tests at Vaeloom verify that services work correctly together across
their defined boundaries. Five integration scenarios are tested: Backend to
Database (CRUD operations with PostgreSQL), Backend AI Logic (agent request
routing with mocked LLM), Backend to Redis (queue publishing and cache
...
For Vaeloom's AI-driven workflows, the backend AI logic integration test is
critical. It verifies that the backend correctly routes documents to the
appropriate AI agent, that the agent processes the request (with a mocked LLM to
avoid non-determinism and API costs), and that the structured response is
correctly returned to the caller. This test catches routing errors, schema
mismatches, and timeout misconfigurations that unit tests would miss.

Contract tests (Pact) between the web frontend and backend serve as an early
warning system for breaking API changes. The frontend team defines the expected
API contract as a Pact file; the backend team runs provider verification against
this contract in CI. If a PR changes the API in a way that breaks the frontend's
expectations, CI fails with a detailed diff of the contract mismatch.
