...
Database optimization at Vaeloom follows a data-driven approach: identify slow
...
This document defines the optimization detection pipeline, common query
anti-patterns, connection pool sizing per service tier, vacuum strategy for
...
> **Diagram:** Database optimization flows from **detection**
> (pg_stat_statements identifies slow queries) through **common optimization
> patterns** (N+1, missing indexes, over-fetching, JSONB abuse), **connection
> pooling** (sized per service tier), **vacuum strategy** (aggressive for
> high-churn, light for read-heavy), and **batch operations** (sized per
> operation type).
...
pool_config = {
    ...
    "worker": {"pool_size": 5, "max_overflow": 1, "pool_timeout": 120},
}
```
...
```python
# Backend connection pool (SQLAlchemy)
...
# Worker pool for batch processing (separate engine)
