...
Grouped by doc domain, EXECUTED LAST to avoid merge conflicts when prior phases
land.
...
| 9.1.4 | `infra(docker): add Dockerfiles for connectors and integrations` | connectors/_/Dockerfile, integrations/_/Dockerfile |
...
| 9.4.2 | `chore(scripts): add utility scripts (docs validation, encoding, mermaid fix)` | scripts/docs__.py, fix__.py         |
...
**Why Phase 1 (Docs) last:** The 260 already-tracked doc files are modified. If
we commit Phase 1 first, every subsequent commit will need to merge against
those modified docs. By staging and committing all code first, then docs last,
we avoid 260+ merge conflicts and redundant tree operations.
