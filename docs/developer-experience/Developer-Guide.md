...
| ------------------------ | -------------------------------------------------------------- |
| `apps/web/`              | Next.js 15 frontend — all pages and components                 |
| `apps/api/`              | FastAPI backend — auth, CRUD, permissions, agents, memory, RAG |
| `packages/shared-types/` | Shared TypeScript type definitions (`@vaeloom/shared-types`)   |
| `packages/ui-kit/`       | Reusable UI components (`@vaeloom/ui-kit`)                     |
| `infra/docker/`          | Docker Compose files for local dev                             |
| `infra/migrations/`      | Database migration files (Alembic)                             |
| `docs/`                  | Project documentation                                          |

## Dev Servers (read this first)

| Task                   | Command (repo root)                             | Notes                                             |
| ---------------------- | ----------------------------------------------- | ------------------------------------------------- |
| Frontend dev           | `pnpm dev:web`                                  | Nx → `@vaeloom/web` only, 2-5s startup            |
| Frontend dev (fastest) | `make dev-web`                                  | `cd apps/web && pnpm next dev` directly           |
| Backend dev            | `pnpm dev:be`                                   | uvicorn via uv (pinned Python 3.12)               |
...
>
> **Backend env:** Pydantic `model_config` has no `env_file`, so export vars in
> your shell; nested settings need double underscores (`DATABASE__URL`).
> `JWT_SECRET` must be 32+ chars or startup refuses.
...
The Developer Guide is the central reference for Vaeloom developers, covering
project navigation across all services, step-by-step workflows for common
development tasks (adding API endpoints, agents, and connectors), and targeted
debugging tips for frequent issues. It serves as the starting point before
diving into service-specific documentation.
