...
> **Purpose:** Guide for contributing to the Vaeloom project **Status:** 🆕 New
...
> **Diagram:** Contribution workflow — **4 getting started steps** → **3
> contribution paths** (bug reports, feature suggestions, pull requests) — each
...
Thank you for considering contributing to Vaeloom. This document outlines the
process for contributing.

## Code of Conduct

All contributors must adhere to our code of conduct: be respectful, assume good
intent, and focus on constructive feedback.

## Getting Started

1. Read the [Setup Guide](./Setup.md) to get the project running locally
2. Read the [Architecture Walkthrough](./Architecture-Walkthrough.md) to
...
1. Check if the feature is already planned (see
...
The Contributing guide defines how developers can contribute to the Vaeloom
project — reporting bugs, suggesting features, and submitting pull requests. It
covers the contribution workflow from discovering issues through merging PRs,
including coding standards, testing requirements, and documentation
expectations.
...
- Define a clear, welcoming contribution process for new and experienced
  contributors
- Establish quality standards for bug reports, feature suggestions, and pull
  requests
...
# Or individual checks (pnpm-only repo — never npm; backend via uv, never bare python/pip/venv)
pnpm lint
pnpm test
pnpm build
cd apps/api && uv run --project apps/api python -m pytest -q -o addopts="" && uv run --project apps/api ruff check .
```
