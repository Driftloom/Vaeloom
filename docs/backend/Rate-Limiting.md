...
graph TD
 ...
 subgraph Endpoints["Endpoint-Specific Limits"]
 direction TB
 ...
 subgraph Implementation["Token Bucket Algorithm"]
 direction TB
 ...
 I4{"tokens > 0?"}
 I5["Consume 1 token<br/>Set last_refill = now<br/>Allow request through"]
 I6["No tokens remaining<br/>Return 429 + Retry-After"]
 end

 subgraph Response["Rate Limit Headers"]
 direction TB
...
> **Diagram:** Rate limiting uses a Redis-backed **token bucket** algorithm per
> user/workspace. Each request checks the token count, refills at the configured
> rate, and either allows (consuming a token) or returns **429 Too Many
> Requests** with `Retry-After`. Tier limits apply across all endpoints;
...
| Forgetting to rate limit internal service calls          | Internal calls between api and ai-service without rate limits can cascade failures — apply internal burst limits for cross-service calls           |
...
1. **Protect system resources** — Prevent any single user, workspace, or API key
   from overwhelming the API or exhausting AI inference capacity
2. **Fair resource allocation** — Ensure Free tier users get predictable
   capacity while Pro and Enterprise users receive proportional allocations
3. **Self-regulating clients** — Return consistent rate limit headers so clients
   can throttle themselves without hitting 429s
4. **Defense against abuse** — Apply tighter limits to expensive endpoints
   (chat, document upload) and auth endpoints to prevent brute force and cost
   exhaustion
...
- Per-tier rate limits: Free (60/min), Pro (300/min), Enterprise (1000/min),
  Internal (5000/min)
- Endpoint-specific tighter limits: chat (20/min), document upload (10/min),
  memory search (30/min), login (5/min)
...
- Per-IP rate limiting for unauthenticated endpoints (not needed — all endpoints
  authenticated)
...
- Rate limiting for internal service-to-service calls (uses separate internal
  tier)
...
sequenceDiagram
 ...
 RL->>RL: Calculate refill: tokens += rate -- (now - T1) / 60s
 RL->>RL: tokens > 0?
...
> **Diagram:** Rate limit flow — Gateway checks rate limit for each request
> against Redis-backed token bucket. Token refill calculated based on elapsed
> time. Under limit: allow with headers; over limit: return 429 with
> Retry-After.
...
> _Last verified: 2026-09-15 — sliding-window limiter with per-endpoint
> decorator + `Retry-After`; local tuning via `RATE_LIMIT_REQUESTS` /
> `RATE_LIMIT_WINDOW`._
