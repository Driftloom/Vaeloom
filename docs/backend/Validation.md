...
> **Purpose:** Define input validation standards for Vaeloom's API **Status:**
> New
...
graph TD
 ...
 subgraph APILayer["API Layer (Pydantic / FastAPI)"]
 direction TB
 ...
 direction TB
 P1["Memory Extract Input:<br/>document_id ˆˆ doc_[a-z0-9]+<br/>content ‰¤ 100K chars<br/>source_type ˆˆ {pdf,docx,email,code}"]
 P2["Agent Action Input:<br/>action ˆˆ allowed list<br/>params match tool schema"]
 end
...
> **Diagram:** Validation runs across three layers. **API Layer** checks input
> shape with Pydantic model validation (required, type, format, length, enum,
> range). **Business Logic Layer** applies domain rules (uniqueness, quotas,
> limits). **AI Agent Input** validates agent-specific schemas with Pydantic.
> Any failure at any layer returns a structured **VALIDATION_ERROR** response
...
| -------------- | ------------------ | ---------------------- |
| API request    | Pydantic (FastAPI) | Input shape validation |
...
| Concern                                     | Mitigation                                                                                                                                                                                                 |
| ------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Validation overhead on large request bodies | Validating a 1MB JSON body with 1000+ fields using Pydantic can take 50-100ms — use partial validation (validate only the fields the handler needs) and skip full validation for trusted internal requests |
...
1. **Defense-in-depth validation** — Validate input at every service boundary
   (API, RPC, event bus, agent tool calls) — never trust any input source
2. **Consistent error responses** — Return the same structured validation error
   format from all layers so clients write one error handler
3. **Fail-fast validation at API layer** — Catch invalid input in middleware
   before it reaches expensive business logic (LLM calls, database writes)
4. **Agent input safety** — Apply the same validation rigor to AI agent inputs
   as user inputs — agent actions can corrupt data just like malicious API calls
...
- 3-layer validation pipeline: API layer (Pydantic), Business Logic (custom
  validators), AI Agent (Pydantic)
- Validation rules: required fields, type checking, length limits, enum
  validation, format validation, range validation, custom domain rules
- Consistent error response format with field-level details
- AI agent input validation using Pydantic models with regex, length, and
  literal constraints

### Out of Scope

- Client-side validation (UX concern, not security — client validation is never
  sufficient)
...
- Business logic validation that requires external service calls (handled by
  service layer)
...
| ----- | ------------------------------------------------------------------------------------------------------------------------ | -------- |
| F-001 | System SHALL validate all API inputs using Pydantic model validation (required, type, length, enum, format, range)       | P0       |
...
sequenceDiagram
 ...
 API->>API: Pydantic: Required fields (name, type)
 API->>API: Pydantic: Type checks (string, UUID)
 API->>API: Pydantic: Length limits (name <= 255)
 API->>API: Pydantic: Enum (DocumentType)
...
> **Diagram:** Validation pipeline — API layer validates input shape with
> Pydantic (required, type, length, enum). On pass, Business Logic layer applies
> domain rules (uniqueness, quotas). Failure at any layer returns the same
...
2. Pydantic model validation check: required fields present, correct types, length limits, valid enums, format patterns, numeric ranges
...
| ----------- | ---------------------------------------------------- | ------------------------ | ----------------------------------------------------------------------------------------------------- |
| Development | Pydantic model validation + custom validator classes | Git push                 | Unit tests: each validator tested with valid + invalid inputs                                         |
...
> _Last verified: 2026-09-15 — Pydantic validation unchanged; MCP configs
> revalidated on every update (see Connectors.md)._
