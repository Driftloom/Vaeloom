...
1. **Define queue** in `apps/api/queues/`
2. **Configure** concurrency, retries, backoff, TTL
3. **Create worker** in `apps/api/workers/`
...
> _Last verified: 2026-09-15 — queue patterns unchanged; durable execution also
> available via `/api/v1/temporal/*` (see API-Reference.md)._
