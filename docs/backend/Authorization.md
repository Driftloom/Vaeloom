...
graph TD
 ...
 subgraph PermissionEngine["Permission Engine (ABAC)"]
 direction TB
 P1["Subject (who)<br/>User ID · Agent name · Role"]
 P2["Resource (what)<br/>Document · Memory · Connector"]
 P3["Action (how)<br/>Read · Write · Act · Delete"]
 P4["Context (when/where)<br/>Time · IP · Workspace"]
 end
 ...
 H1["User--> Workspace--> Connectors<br/>--> Permissions per (Connector × Agent × Action)"]
 end

 subgraph Autonomy["Agent Autonomy Model"]
 A1["Default: Suggest-mode<br/>Propose--> User approves"]
...
> error). **Code truth:** ABAC evaluate/levels/hierarchy/autonomy endpoints and
> `permission_assignments/autonomy_tracker` tables are spec-only — none exist in
> code. What exists: `viewer/editor/admin` RBAC DI helpers
> (`middleware/rbac.py:29-51`, never mounted as middleware) + per-route auth +
> the agent approval gate below (18 base gated tools + dynamic `mcp__*`,
> `tools/executor.py:261-300`; HMAC ledger + single-use consume,
> `orchestrator/loop.py:181-379,873-950`).

## Approval / human-in-the-loop (implemented)

```mermaid
stateDiagram-v2
  [*] --> Decide: agent proposes tool call
  Decide --> Execute: no approval required
  Decide --> Pending: approval required
  Pending --> Execute: user approves
  Pending --> [*]: user rejects
  Execute --> [*]: audit appended
```

> **Diagram:** `paused_awaiting_approval` checkpoint in Supervisor
> (`supervisor.py:244-264,374-409`), resume via `resume_supervisor`; PENDING
> surfaced as proposal cards with `approval_id` (`router.py:761-778`,
> `loop.py:126-169`). Fail-closed on missing/expired/ tampered approval.
...
sequenceDiagram
 ...
 PE->>PE: Check autonomy level--> auto-execute allowed
