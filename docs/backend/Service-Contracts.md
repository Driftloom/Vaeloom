...
This document is the single source of truth for the internal API. Every change
to the contract must go through this document first; every deployment must
verify contract compatibility.

## Goals

- Define the RPC protocol (HTTP (internal calls) with protobuf)
...
> RPC methods. Pact verifies consumer expectations against provider
> implementations on every CI run.

## RPC Protocol

> **Code truth (2026-09-15):** the backend is a single monolithic FastAPI app —
> modules communicate via **direct in-process function calls**, not gRPC. There
> is no protobuf transport, no mTLS between services, and no Pact broker in the
> repo. The table below is retained as the interface-evolution convention
> (versioned Pydantic models, timeouts, idempotency); read "RPC method" as
> "module public-interface method" and "protobuf schema" as "Pydantic model".
...
Contract testing strategy (consumer-driven via Pact):
  ...
  1. API service (consumer) defines expectations:
     ...
     - Written as Pact interactions in Python
...
> _Last verified: 2026-09-15 — monolith note added: in-process calls, no
> gRPC/Pact; Pydantic models are the contract._
