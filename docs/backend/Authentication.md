...
| ----------- | ---------------------- | ---------------------------------------------------------------------------------------------------------- |
| MVP         | Email/password + OAuth | Self-issued bcrypt + JWT, HS256 (`config.py:53-56`) — Clerk/Auth0 references below are stale design intent |
| Enterprise  | + SAML/OIDC SSO        | Same service, enterprise tier                                                                              |
...
user login through OAuth, JWT issuance, to an authenticated API call with
...
sequenceDiagram
 ...
 Web-->>User: Redirect to dashboard
  ...
  Note over Web,Auth: Session established (1h access + 30d refresh, HS256 - code truth config.py:53-56; doc claims of 24h/15m elsewhere are stale)
...
> via Clerk/Auth0 with OAuth; API clients use JWT access tokens. Enterprise
> layer adds SAML/OIDC SSO and SCIM provisioning. Tokens stored in httpOnly
> cookies (web), secure storage (mobile), or Secrets Manager (connector tokens).
> Session metadata cached in Redis for fast lookup.
...
| JWT Service     | FastAPI + PyJWT                       | Token issuance, RS256 signing, JWKS caching    |
| Refresh Service | FastAPI + Redis                       | Token rotation, invalidation, refresh grant    |
