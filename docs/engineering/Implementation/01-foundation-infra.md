...
1. Create a reproducible monorepo scaffold with three services, shared types,
   and infrastructure configuration
2. Establish a CI pipeline (lint, typecheck, test, build) that gates every PR
   before merge
3. Implement auth (email/password signup + login) with a structured interface
   for future OAuth/SSO replacement
4. Provide a local development environment via Docker Compose that does not
   require any cloud accounts
...
- Docker Compose local dev environment with Postgres, Redis, and both services (hot reload)
