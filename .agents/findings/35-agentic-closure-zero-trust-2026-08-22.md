...
| --------- | ---------- | ------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **AC-01** | **HIGH**   | `loop.py:379` `agent_scopes=[td.required_scope]`                          | ReAct permission self-grant — executor check is no-op. LLM can call any offered tool. Violates PATI least-privilege / OWASP LLM06. See §1 G2.                                                                                                                                                                                                                              |
| **AC-02** | **HIGH**   | `router.py:235` `if _is_multi...` before MVP gate                         | Supervisor bypasses `mvp_scope_enforced` — enterprise agents reachable via multi-intent.                                                                                                                                                                                                                                                                                   |
| **AC-03** | **MEDIUM** | `background_daemon.py:30` `SELECT Workspace.id LIMIT 20`                  | Watchers iterate without tenant scoping; `GmailAgent` called with dummy, not real `GmailClient.fetch_emails`.                                                                                                                                                                                                                                                              |
| **AC-04** | **MEDIUM** | `ChatWindow.tsx:500` `onSseEvent` + fallback                              | Duplicated blocking-chat logic (100 lines) as fallback — maintainability, drift risk.                                                                                                                                                                                                                                                                                      |
| **AC-05** | **MEDIUM** | `loop.py:188` `generate_embedding` per plan                               | RAG pays embedding latency on every request, even when no vector results. No `QDRANT_URL`/tenant check before call.                                                                                                                                                                                                                                                        |
| **AC-06** | **MEDIUM** | `approval.py:322` dedup `SELECT ... WHERE canonical_name=:name`           | Preference dedup by name only — same rejection note for different `action_type` collides; `REJECTED` vs `APPROVED` same name deduped incorrectly.                                                                                                                                                                                                                          |
| **AC-07** | **LOW**    | `supervisor.py:30` `context[aname]=summary`                               | Cross-agent context is string concatenation — prompt injection in one agent's summary can steer next agent (no provenance tagging).                                                                                                                                                                                                                                        |
| **AC-08** | **LOW**    | `tests/test_agentic_gaps_closure.py:1` `pytestmark=asyncio` on sync tests | 14 `PytestWarning` for sync tests marked asyncio — noise hides real warnings.                                                                                                                                                                                                                                                                                              |
| **AC-09** | **MEDIUM** | `pytest -n 4` full suite                                                  | Full suite with `512` ReAct budget? Actually hang observed: `pytest -q -n 4` exceeds 10 min (killed at 600s/900s) on this branch, while `collect-only` is 7s. Subsets pass. Indicates resource exhaustion / DB contention with new RAG vector SQL + daemon `async_session_factory` per plan. Not reproduced without `agentic` changes (prior branch `0feb7ff` not tested). |
| **AC-10** | **LOW**    | `pyproject.toml` `croniter` added                                         | `uv.lock` updated, but `uv sync --no-install-project` showed `croniter-6.2.4` installed via `pip` not `uv pip` — ensure `uv lock` + `uv sync` path, not `pip`.                                                                                                                                                                                                             |
| **AC-11** | **INFO**   | `git` history                                                             | `e92f352` advances 6 commits during audit — `EXECUTION-STATUS.md` last line still references `e1da219`, 1 commit behind.                                                                                                                                                                                                                                                   |

No P0-CRITICAL. Two HIGH self-flags are honest about this session's own
shortcuts.
