...
class TestFileSecurity:
    def test_sanitize_filename(self):
        ...
        assert file_security_service.sanitize_filename("../../etc/passwd.pdf") == "passwd.pdf"
        assert file_security_service.sanitize_filename("..\\..\\windows\\system32.exe") == "system32.exe"
        assert file_security_service.sanitize_filename("bad\x00file.txt") == "badfile.txt"
        assert file_security_service.sanitize_filename("  spaces  .png  ") == "spaces.png"
        assert file_security_service.sanitize_filename("") == "uploaded_file"
    ...
    def test_disallowed_extension_rejection(self):
        verdict = file_security_service.inspect_file("payload.exe", b"MZ\x90\x00", "application/x-msdownload")
        assert verdict.is_safe is False
        assert verdict.scan_status == "MALICIOUS"
        assert "Extension '.exe' is not permitted" in verdict.rejection_reason
    ...
    def test_spoofed_pdf_with_executable_payload(self):
        ...
        assert verdict.is_safe is False
        assert verdict.scan_status == "MALICIOUS"
        assert "Dangerous executable or script binary detected" in verdict.rejection_reason
    ...
    def test_eicar_malware_quarantine(self):
        ...
        assert verdict.is_safe is False
        assert verdict.scan_status == "MALICIOUS"
        assert "EICAR test malware signature detected" in verdict.rejection_reason
