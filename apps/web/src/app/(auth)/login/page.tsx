...
function LoginForm() {
  ...
  async function onMfaSubmit(e: React.FormEvent) {
    ...
    if (!mfaCode || mfaCode.trim().length < 6) {
      setErrors({ form: 'Please enter a valid 6-digit code or recovery code' });
      return;
    }
    setSubmitting(true);
    setErrors({});
    try {
      if (!mfaChallenge?.token) throw new Error('MFA challenge token missing');
      await verifyMfa(mfaChallenge.token, mfaCode.trim());
      await navigateAfterAuth();
    } catch (err: any) {
      const message = err instanceof ApiError ? err.message : err?.message || 'Verification failed';
      setErrors({ form: message });
    } finally {
      setSubmitting(false);
    }
              ...
              <form onSubmit={onMfaSubmit} className="space-y-5" suppressHydrationWarning>
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full text-center text-xl font-mono py-3 rounded-xl border border-border bg-background text-text focus:border-primary focus:outline-none"
                  />
                </div>
