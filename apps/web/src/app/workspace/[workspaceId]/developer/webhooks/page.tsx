...
const eventOptions = [
  'job.match', 'application.submitted', 'agent.task.completed',
  'memory.created', 'workspace.updated', 'user.joined',
  'billing.invoice', '*',
];
...
export default function WebhooksPage() {
  ...
  const fetchDeliveries = useCallback(async (webhookId: string) => {
    try {
      const res = await api.request<{ deliveries: DeliveryItem[] }>(`/webhooks/${webhookId}/deliveries`);
  ...
  const toggleActive = useCallback(async (webhook: WebhookItem) => {
    try {
  ...
  const testFire = useCallback(async (webhookId: string) => {
    try {
  ...
  const viewDeliveries = useCallback(async (webhook: WebhookItem) => {
  ...
  const webhookColumns: Column<WebhookItem>[] = [
    ...
    { key: 'url', header: 'URL', render: (w) => <code className="text-xs font-mono text-text-muted truncate max-w-[200px] block">{w.url}</code> },
    { key: 'events', header: 'Events', render: (w) => (
      <div className="flex flex-wrap gap-1">
        {w.events.slice(0, 3).map(e => (
          <span key={e} className="text-xs font-mono px-1.5 py-0.5 rounded bg-surface border border-border text-text-muted">{e}</span>
        ))}
        {w.events.length > 3 && <span className="text-xs text-text-muted">+{w.events.length - 3}</span>}
      </div>
    )},
    { key: 'active', header: 'Active', render: (w) => (
      <Toggle enabled={w.active} onChange={() => toggleActive(w)} />
    )},
    { key: 'id', header: '', render: (w) => (
      <div className="flex gap-2 justify-end">
        <Button variant="secondary" size="sm" onClick={() => testFire(w.id)}>Test</Button>
        <Button variant="secondary" size="sm" onClick={() => viewDeliveries(w)}>Deliveries</Button>
        <Button variant="ghost" size="sm" onClick={() => deleteWebhook(w.id)} className="text-accent">Delete</Button>
      </div>
  ...
  return (
    <div className="space-y-8">
      ...
      <Card padding="lg">
        {loading ? (
          ...
          <div className="text-center py-8">
            ...
            <Button variant="secondary" onClick={fetchWebhooks}>Retry</Button>
          </div>
        ) : webhooks.length === 0 ? (
          <EmptyState
      ...
      {showDeliveries && selectedWebhook && (
        <Card padding="lg">
          <div className="flex justify-between items-center mb-4">
            ...
            <Button variant="secondary" size="sm" onClick={() => setShowDeliveries(false)}>Close</Button>
          </div>
          {deliveries.length === 0 ? (
            ...
            <div className="space-y-2">
              {deliveries.map(d => (
                <div key={d.id} className="flex items-center justify-between p-3 bg-background rounded-lg border border-border text-sm">
      ...
      <Modal isOpen={showCreate} onClose={() => setShowCreate(false)} title="Create Webhook">
        <div className="space-y-4">
          <Input label="Name" value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="e.g. Production" />
          <Input label="URL" value={newUrl} onChange={(e) => setNewUrl(e.target.value)} placeholder="https://hooks.example.com/events" />
          <Input label="Secret" value={newSecret} onChange={(e) => setNewSecret(e.target.value)} placeholder="Leave blank to auto-generate" />
          <div className="space-y-1">
            <label className="block text-sm font-medium text-text">Events</label>
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button onClick={createWebhook} disabled={!newName || !newUrl}>Create</Button>
          </div>
        </div>
      </Modal>
    </div>
