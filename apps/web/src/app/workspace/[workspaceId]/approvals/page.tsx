    ...
    <div className="flex flex-col h-full">
      <header className="mb-6 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-text-muted text-sm">Unified inbox for all agent suggestions requiring approval. Least-privilege — review scopes, risk, and expiry before acting. Use <kbd className="font-mono">A</kbd> to approve, <kbd className="font-mono">R</kbd> to reject.</p>
        </div>
      ...
      <TabPanel id={active} activeTab={active}>
        ...
        : filtered.length === 0 ? <EmptyState title={active === 'pending' ? 'No pending approvals' : `No ${active} approvals`} description={active === 'pending' ? 'Agent suggestions will appear here — Files renames, Gmail drafts, Schedule events, Job applications. All are reversible via History.' : `No ${active} items match the current filter.` } />
        : (
