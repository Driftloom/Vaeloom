    ...
    <header className="h-14 border-b border-border bg-surface flex items-center justify-between px-6 shrink-0">
      ...
      <div className="flex items-center gap-4">
        <button onClick={() => setPaletteOpen(true)} className="hidden sm:flex items-center gap-2 rounded-full border border-border bg-background px-3 py-1.5 text-xs text-text-muted hover:text-text hover:border-primary/30 transition-colors" aria-label="Global search">
          <span>Search</span><kbd className="font-mono text-[10px] border border-border rounded px-1">⌘K</kbd>
        </button>
        <button onClick={() => setPaletteOpen(true)} className="sm:hidden p-2 rounded-lg border border-border hover:bg-surface-hover" aria-label="Search">⌕</button>
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-[20vh] p-4">
          ...
          <div className="relative w-full max-w-xl bg-surface border border-border rounded-xl shadow-2xl overflow-hidden">
            ...
            <div className="max-h-80 overflow-auto p-2">
              {searching ? <p className="p-4 text-center text-sm text-text-muted">Searching…</p>
              : results.length === 0 ? <p className="p-6 text-center text-sm text-text-muted">{query.trim() ? 'No results — try different keywords.' : 'Type to search across files, memories, and events grouped by source + score.'}</p>
              : (
            ...
            <p className="px-4 py-2 text-xs text-text-dim border-t border-border">Enter opens · ↑↓ navigates · Global search is workspace-aware.</p>
          </div>
        </div>
      )}
    </header>
