  ...
  return (
    <div className="rounded-xl border border-border bg-surface p-6 shadow-sm">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-border pb-4 mb-5">
        <div>
          <h2 className="text-lg font-semibold text-text">Active Login Sessions</h2>
        ...
        {otherSessions.length > 0 && (
          <button
            ...
            className="inline-flex items-center justify-center px-3.5 py-1.5 text-xs font-medium rounded-lg border border-red-500/30 text-red-500 hover:bg-red-500/10 transition-colors disabled:opacity-50"
          >
                ...
                <div className="flex items-center gap-3.5 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {session.isCurrent && (
                        <span className="inline-flex items-center px-2 py-0.5 text-[11px] font-semibold rounded-full bg-emerald-500/15 text-emerald-400 border border-emerald-500/30">
                          Current Device
                        </span>
                      )}
                    </div>
                ...
                <div>
                  {!session.isCurrent ? (
                    <button
                      ...
                      className="px-3 py-1 text-xs font-medium rounded border border-border hover:border-red-500/40 hover:text-red-400 transition-colors disabled:opacity-50"
                    >
                      {revokingId === session.id ? 'Revoking...' : 'Revoke'}
                    </button>
                  ) : (
                    <span className="text-xs font-mono text-emerald-400/80 pr-2">Active now</span>
                  )}
                </div>
              </div>
            );
