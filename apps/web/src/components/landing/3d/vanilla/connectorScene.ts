...
export function createConnectorFlow(
  theme: 'dark' | 'light',
  density: number,
): {
  group: THREE.Group;
  update: (t: number, dt: number, rm: boolean) => void;
  dispose: () => void;
} {
  const palette = scenePalette(theme);
  ...
  const streams = SOURCES.map((color, si) => {
    ...
    pts.frustumCulled = false;
    group.add(pts);
    return { angle, color, node, geo, pts, progress, seeds };
  });
  const linkGeo = new THREE.BufferGeometry();
  linkGeo.setAttribute(
    'position',
  ...
  function update(t: number, dt: number, rm: boolean): void {
    const tt = rm ? 0 : t;
    ring.rotation.y = tt * 0.1;
    coreHalo.material.opacity = 0.45 + Math.sin(tt * 1.3) * 0.1;
    const linkPos = (linkGeo.getAttribute('position') as THREE.BufferAttribute)
      .array as Float32Array;
    streams.forEach((st, si) => {
      const a = st.angle + tt * 0.1;
      const nx = Math.cos(a) * RING_RADIUS;
      const nz = Math.sin(a) * RING_RADIUS;
      st.node.position.set(nx, Math.sin(tt * 1.2 + si * 1.4) * 0.12, nz);
      linkPos.set([nx, st.node.position.y, nz, Math.cos(a) * 1.35, 0, Math.sin(a) * 1.35], si * 6);
      const attr = st.geo.getAttribute('position') as THREE.BufferAttribute;
      const arr = attr.array as Float32Array;
      for (let i = 0; i < perSource; i++) {
        let p = st.progress[i]! + dt * (0.35 + (i % 5) * 0.05);
