...
class TestAgentRequest:
    def test_construction_with_explicit_name(self):
        from backend.orchestrator.loop import AgentRequest
...
class TestReflectResult:
    def test_construction(self):
        from backend.orchestrator.loop import ReflectResult
...
class TestPlanPhase:
    async def test_returns_plan_dict(self):
        from backend.orchestrator.loop import plan_phase, AgentRequest
        from backend.orchestrator.state import LoopState
...
class TestActPhase:
    ...
    async def test_dispatch_paths(self, agent_type, method_name, expected_action):
        from backend.orchestrator.loop import act_phase, AgentRequest
...
class TestObservePhase:
    async def test_returns_observe_dict(self):
        from backend.orchestrator.loop import observe_phase
...
class TestReflectPhase:
    ...
    async def test_reflect_paths(self, action, confidence, iteration, expected_satisfied, expected_reason_substr):
        from backend.orchestrator.loop import reflect_phase, AgentRequest
        agent = _make_agent("TestAgent")
        req = AgentRequest(agent, "r1", "msg", "ws1")
...
class TestImprovePhase:
    async def test_returns_latest_observe_summary(self):
        from backend.orchestrator.loop import improve_phase, AgentRequest
        from backend.orchestrator.state import LoopState
        agent = _make_agent("TestAgent")
        req = AgentRequest(agent, "r1", "msg", "ws1")
        state = LoopState("r1")
...
class TestEscalate:
    async def test_returns_escalated(self):
        from backend.orchestrator.loop import escalate_to_user
        from backend.orchestrator.state import LoopState
        state = LoopState("r1")
...
class TestRunAgentLoop:
    async def test_satisfied_on_first_iteration(self, tmp_path, monkeypatch):
        from backend.orchestrator.loop import run_agent_loop, AgentRequest
    ...
    async def test_max_iterations_returns_success(self, tmp_path, monkeypatch):
        from backend.orchestrator.loop import run_agent_loop, AgentRequest, act_phase
        monkeypatch.setenv("VAELOOM_STATE_DIR", str(tmp_path))
        ...
        from backend.orchestrator import loop as loop_module
    ...
    async def test_max_iterations_escalates(self, tmp_path, monkeypatch):
        from backend.orchestrator.loop import run_agent_loop, AgentRequest, reflect_phase
        monkeypatch.setenv("VAELOOM_STATE_DIR", str(tmp_path))
...
class TestStatePersistence:
    async def test_load_or_create_new(self, tmp_path, monkeypatch):
        monkeypatch.setenv("VAELOOM_STATE_DIR", str(tmp_path))
        from backend.orchestrator.state import load_or_create_state
    ...
    async def test_load_or_create_existing(self, tmp_path, monkeypatch):
        monkeypatch.setenv("VAELOOM_STATE_DIR", str(tmp_path))
        from backend.orchestrator.state import load_or_create_state, save_checkpoint, LoopState
    ...
    async def test_load_or_create_corrupt_json(self, tmp_path, monkeypatch, caplog):
        ...
        monkeypatch.setenv("VAELOOM_STATE_DIR", str(tmp_path))
        from backend.orchestrator.state import STATE_DIR
    ...
    async def test_save_checkpoint_writes_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("VAELOOM_STATE_DIR", str(tmp_path))
        from backend.orchestrator.state import save_checkpoint, LoopState, STATE_DIR
    ...
    async def test_save_checkpoint_handles_oserror(self, monkeypatch, caplog):
        ...
        try:
            ...
            monkeypatch.setattr("backend.orchestrator.state.STATE_DIR", original_state_dir)
