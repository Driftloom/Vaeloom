...
class TestComputeEmbedding:
    async def test_returns_embedding_from_llm(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "backend.services.recommendation_service.llm_service",
    ...
    async def test_returns_zero_vector_on_failure(self, monkeypatch):
        ...
        monkeypatch.setattr("backend.services.recommendation_service.llm_service", llm_mock)
        svc = RecommendationService()
...
class TestGenerate:
    async def _make_dto(self, **overrides):
        from backend.schemas.recommendation import GenerateRecommendationRequest
...
class TestRecordFeedback:
    async def test_existing_recommendation_inserts_feedback(self):
        ...
        from backend.schemas.recommendation import FeedbackRequest
