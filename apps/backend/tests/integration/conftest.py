...
def _build_test_app(db_session):
    import backend.infrastructure.metrics
...
async def auth_headers(client: AsyncClient) -> dict:
    res = await client.post(
        "/api/v1/auth/signup",
        json={"email": "int-user@test.com", "password": "IntTest1234!"},
    )
...
async def secondary_auth_headers(client: AsyncClient) -> dict:
    ...
    res = await client.post(
        "/api/v1/auth/signup",
        json={"email": "int-user-b@test.com", "password": "IntTest1234!"},
    )
    assert res.status_code == 201
    token = res.json()["access_token"]
    return {"Authorization": f"Bearer {token}"}
...
async def mock_connector_test(monkeypatch):
    ...
    monkeypatch.setattr(
        ConnectorExtService, "test_connection", fake_test_connection
    )


@pytest_asyncio.fixture(autouse=True)
async def fix_sqlite_timezone(monkeypatch):
    """Make refresh_token comparison work with SQLite (naive datetimes)."""
    ...
    async def fixed_refresh(self, refresh_token, db=None):
        ...
        try:
            ...
            if "can't compare offset-naive and offset-aware" in str(e):
                ...
                from backend.models.schema import AuthSession, User
                from backend.schemas.auth import AuthResponse, PublicUser
                ...
                if session.status != "ACTIVE" or expires < now:
                    from fastapi import HTTPException
    ...
    monkeypatch.setattr(auth_mod.auth_service, "refresh_token", fixed_refresh.__get__(auth_mod.auth_service))
