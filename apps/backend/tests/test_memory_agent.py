...
class TestCosineSimilarity:
    def test_identical_vectors(self):
        from backend.agents.memory_agent.retrieval import _cosine_similarity
        v = [0.5, 0.5, 0.5, 0.5]
...
class TestVectorSearch:
    ...
    async def test_pg_vector_path_entity_source(self, monkeypatch):
        ...
        monkeypatch.setattr("backend.database.async_session_factory", factory)

        from backend.agents.memory_agent.retrieval import vector_search
    ...
    async def test_import_error_returns_empty(self, monkeypatch):
        _clear_cached_imports(monkeypatch, "backend.database")
        from backend.agents.memory_agent.retrieval import vector_search
        results = await vector_search("test", str(uuid.uuid4()), 5)
        assert results == []
    ...
    async def test_embedding_exception_falls_back_to_in_memory(self, monkeypatch):
        ...
        session, factory = _make_session_and_factory(execute_side_effect=dispatcher)
        monkeypatch.setattr("backend.database.async_session_factory", factory)
        monkeypatch.setattr("backend.services.llm_service.llm_service.generate_embedding", AsyncMock(side_effect=Exception("API error")))
        from backend.agents.memory_agent.retrieval import vector_search
        results = await vector_search("test", str(uuid.uuid4()), 5)
        assert len(results) == 1
        assert "Vector search unavailable" in results[0].content
...
class TestFallbackVectorSearch:
    ...
    async def test_scores_and_returns_top_k(self):
        from backend.agents.memory_agent.retrieval import _fallback_vector_search
...
class TestInMemoryVectorSearch:
    def test_returns_fallback_result(self):
        from backend.agents.memory_agent.retrieval import _in_memory_vector_search
...
class TestKeywordSearch:
    ...
    async def test_entity_matching_via_ilike(self, monkeypatch):
        ...
        from backend.agents.memory_agent.retrieval import keyword_search
...
class TestGraphTraversal:
    ...
    async def test_entity_matching(self, monkeypatch):
        ...
        from backend.agents.memory_agent.retrieval import graph_traversal
...
class TestRerank:
    async def test_dedup_by_id(self):
        from backend.agents.memory_agent.retrieval import RetrievedMemory, rerank
...
class TestRetrieve:
    ...
    async def test_vector_strategy(self, monkeypatch):
        ...
        from backend.agents.memory_agent.retrieval import retrieve
...
class TestFuzzyScore:
    def test_exact_match(self):
        from backend.agents.memory_agent.merge import _fuzzy_score
...
class TestComputeConfidence:
    def test_same_type_gets_boost(self):
        from backend.agents.memory_agent.merge import _compute_confidence
...
class TestMergeCheck:
    ...
    async def test_merge_action_when_confidence_high(self, monkeypatch):
        ...
        from backend.agents.memory_agent.merge import merge_check
...
class TestFallbackMergeCheck:
    def test_react_returns_merge(self):
        from backend.agents.memory_agent.merge import _fallback_merge_check
