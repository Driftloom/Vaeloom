...
class TestParsers:
    ...
    async def test_base_parser_not_implemented(self):
        from backend.ingestion.parsers import BaseParser
        parser = BaseParser()
    ...
    async def test_docx_parser_success(self):
        from backend.ingestion.parsers import DOCXParser
    ...
    async def test_image_parser_success(self, monkeypatch):
        from backend.ingestion.parsers import ImageParser
    ...
    async def test_parse_document_unknown_ext(self):
        from backend.ingestion.parsers import parse_document, UnsupportedFormatError
    ...
    async def test_parse_document_timeout(self, monkeypatch):
        import backend.ingestion.parsers as parsers_module
...
class TestDedup:
    ...
    async def test_compute_content_hash(self):
        from backend.ingestion.dedup import compute_content_hash
    ...
    async def test_check_dedup_version_match(self, monkeypatch):
        from backend.ingestion.dedup import check_dedup
        ...
        monkeypatch.setattr("backend.database.async_session_factory", mock_factory)
    ...
    async def test_fallback_dedup_duplicate(self):
        from backend.ingestion.dedup import _fallback_dedup
...
class TestPipeline:
    ...
    async def test_pipeline_new_doc(self, monkeypatch):
        from backend.ingestion.parsers import ParsedDocument
        ...
        monkeypatch.setattr("backend.ingestion.pipeline.parse_document", mock_parse)
        monkeypatch.setattr("backend.ingestion.pipeline.check_dedup", mock_dedup)
    ...
    async def test_pipeline_unsupported_format(self, monkeypatch):
        from backend.ingestion.parsers import UnsupportedFormatError
        from backend.ingestion.pipeline import run_pipeline

        async def mock_parse_error(filename, content):
            raise UnsupportedFormatError("No parser for .xyz")

        monkeypatch.setattr("backend.ingestion.pipeline.parse_document", mock_parse_error)
