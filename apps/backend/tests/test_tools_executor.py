...
from backend.tools.definitions import ToolDefinition
...
async def test_execute_search_documents_success(monkeypatch):
    ...
    monkeypatch.setattr("backend.database.async_session_factory", lambda: session)
...
async def test_execute_search_documents_import_error(monkeypatch):
    ...
    monkeypatch.delattr("backend.database.async_session_factory")
...
async def test_execute_search_gmail_success(monkeypatch):
    ...
    monkeypatch.setattr("backend.clients.gmail_client.GmailClient", MockGmail)
...
async def test_execute_search_jobs_with_keyword_list(monkeypatch):
    ...
    monkeypatch.setattr("backend.clients.job_board_client.JobBoardClient", MockClient)
...
async def test_execute_list_calendar_events_success(monkeypatch):
    ...
    monkeypatch.setattr("backend.clients.calendar_client.CalendarClient", MockClient)
...
async def test_execute_draft_email_success(monkeypatch):
    ...
    monkeypatch.setattr("backend.clients.gmail_client.GmailClient", MockClient)
...
async def test_execute_tool_success(monkeypatch):
    ...
    monkeypatch.setattr("backend.tools.executor.check_permission", mock_check)
    monkeypatch.setattr("backend.tools.executor._audit_log", lambda *a: None)
...
async def test_execute_tool_unknown_tool_fallback(monkeypatch):
    ...
    monkeypatch.setattr("backend.tools.executor._execute_mock", mock_fallback)
...
async def test_execute_tool_audit_log_on_success(monkeypatch):
    ...
    monkeypatch.setattr("backend.tools.executor._audit_log", fake_audit)
...
def test_get_tools_for_agent_returns_matching_tools():
    from backend.tools.definitions import get_tools_for_agent, ALL_TOOLS
...
def test_get_tools_for_agent_skips_unknown():
    from backend.tools.definitions import get_tools_for_agent
...
async def test_execute_search_gmail_import_error(monkeypatch):
    import backend.clients.gmail_client
    monkeypatch.delattr("backend.clients.gmail_client.GmailClient")
...
async def test_execute_search_jobs_import_error(monkeypatch):
    import backend.clients.job_board_client
    monkeypatch.delattr("backend.clients.job_board_client.JobBoardClient")
...
async def test_execute_list_calendar_events_import_error(monkeypatch):
    import backend.clients.calendar_client
    monkeypatch.delattr("backend.clients.calendar_client.CalendarClient")
    result = await _execute_list_calendar_events({"start_date": "", "end_date": ""}, WS_ID)
    assert result["status"] == "error"
