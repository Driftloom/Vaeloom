...
from backend.migrations import MIGRATIONS, downgrade_to, run_migrations

pytestmark = pytest.mark.asyncio

EXPECTED_VERSIONS = {"0003_approvals", "0004_memory_taxonomy", "0005_rls", "0006_idempotency"}
...
class TestMigrationRunner:
    ...
    async def test_downgrade_partial(self, migration_engine):
        ...
        assert set(rolled_back) == {"0006_idempotency", "0005_rls"}
    ...
    async def test_rls_noop_on_sqlite(self, migration_engine):
        import importlib
        rls_module = importlib.import_module("backend.migrations.0005_rls")
        async with migration_engine.begin() as conn:
