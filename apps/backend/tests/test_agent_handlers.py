...
# =============================================================================
from backend.agents.gmail_agent.handler import GmailAgent, ClassifiedEmail
...
class TestGmailAgent:
    ...
    async def test_get_client_initializes_when_none(self, monkeypatch):
        ...
        monkeypatch.setattr("backend.clients.gmail_client.GmailClient", lambda: fake_client)
        agent = GmailAgent()
        agent._client = None
    ...
    async def test_classify_llm_path(self, monkeypatch):
        ...
        monkeypatch.setattr("backend.agents.gmail_agent.handler.llm_service.generate_completion", fake_llm)
...
# =============================================================================
from backend.agents.resume_agent.handler import ResumeAgent
...
class TestResumeAgent:
    ...
    async def test_llm_generate_bullet_success(self, monkeypatch):
        ...
        monkeypatch.setattr("backend.agents.resume_agent.handler.llm_service.generate_completion", AsyncMock(return_value={"content": "Built amazing feature"}))
        agent = ResumeAgent()
        result = await agent._llm_generate_bullet("Built feature", "Dev", "Co")
    ...
    async def test_llm_generate_bullet_exception(self, monkeypatch):
        monkeypatch.setattr(settings, "llm_api_key", "key")
        monkeypatch.setattr("backend.agents.resume_agent.handler.llm_service.generate_completion", AsyncMock(side_effect=Exception("LLM down")))
        agent = ResumeAgent()
        result = await agent._llm_generate_bullet("Built feature", "Dev", "Co")
        assert result == "Built feature at Co as Dev"
...
# =============================================================================
from backend.agents.memory_agent.handler import MemoryAgentHandler
...
class TestMemoryAgentHandler:
    ...
    async def test_execute(self, monkeypatch):
        from backend.agents.memory_agent.extraction import ExtractedFacts, ExtractedEntity, ExtractedRelationship
        agent = MemoryAgentHandler()
        ...
        monkeypatch.setattr("backend.agents.memory_agent.handler.extract", fake_extract)
        monkeypatch.setattr("backend.agents.memory_agent.handler.merge_check", fake_merge_check)
...
# =============================================================================
from backend.agents.reflection_agent.handler import ReflectionAgent
...
# =============================================================================
from backend.agents.reminder_agent.handler import ReminderAgent
...
# =============================================================================
from backend.agents.security_agent.handler import SecurityAgent
...
# =============================================================================
from backend.agents.analytics_agent.handler import AnalyticsAgent
...
# =============================================================================
from backend.agents.connector_agent.handler import ConnectorAgent
...
# =============================================================================
from backend.agents.plugin_agent.handler import PluginAgent
...
# =============================================================================
from backend.agents.coding_agent.handler import CodingAgent
...
# =============================================================================
from backend.agents.github_agent.handler import GitHubAgent
...
# =============================================================================
from backend.agents.learning_agent.handler import LearningAgent
...
# =============================================================================
from backend.agents.recommendation_agent.handler import RecommendationAgent
