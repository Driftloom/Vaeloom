...
import {
  CalendarIntegration,
  parseCalendarConfig,
} from '@vaeloom/integration-calendar';
...
const connection = await calendar.connect({
  provider: 'calendar',
  settings: config,
});
...
await calendar.updateEvent(connection.connectionId, 'primary', created.id, {
...
// Watch
await calendar.watchCalendar(
  connection.connectionId,
  'primary',
  'https://app.vaeloom.com/webhooks/calendar',
  'chan-1',
  'tok-1',
);
...
registered `channelId`/`subscriptionId` and `resourceToken`/`clientState` with a
