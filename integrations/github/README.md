...
GitHub integration for Vaeloom. Lists repositories, reads/writes issues and pull
requests, fetches commits, registers webhooks, and syncs activity into Vaeloom
memories.
...
- **OAuth App**: set `clientId`, `clientSecret`, `redirectUri`, and
  `webhookSecret`.
...
import {
  GithubIntegration,
  parseGithubConfig,
} from '@vaeloom/integration-github';
...
await github.createWebhook(
  connection.connectionId,
  'octocat/hello',
  'https://app.vaeloom.com/webhooks/github',
);

// OAuth flow
const authClient = github.buildAuthClient({
  provider: 'github',
  settings: config,
});
