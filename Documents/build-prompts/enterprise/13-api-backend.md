...
- **Webhooks:** let external systems subscribe to Vaeloom events (extends the internal event bus from `mvp/05`) — e.g. `application.status_changed`, `memory.updated` — with signed payloads and retry-with-backoff delivery.
