...
- **Formal threat model:** document Vaeloom's threat model explicitly against the OWASP Top 10 and AI-specific risks (prompt injection — already mitigated in MVP, now formally documented and tested against a larger adversarial suite; data leakage across tenants; tool misuse). This is a deliverable (a document), not just code — enterprise customers will ask for it directly.
