...
Do **not** assume `MVP-P16` passed because a document says “complete.” Re-audit its actual artifacts and evidence before executing this phase.
...
- `DEL-MVP-P16-03` — SBOM/provenance/signatures; versioned, owned, reviewed and linked.
- `DEL-MVP-P16-04` — deployment/rollback; versioned, owned, reviewed and linked.
- `DEL-MVP-P16-05` — environment evidence; versioned, owned, reviewed and linked.
...
On `NO-GO`, switch to `AUDIT_COMPLETED_PHASE` then `REMEDIATE_FAILED_PHASE` for `MVP-P16`. Re-run its full gate and generate a new handoff before resuming this file.
...
## 1. Mission
Implement telemetry, SLOs, alerts, dashboards, runbooks, incident response, support and cost visibility.
...
- **Phase-specific rule:** Trace user→workspace→job→agent→tool→model→memory→approval→action.
...
- Apply phase rule: Trace user→workspace→job→agent→tool→model→memory→approval→action.
...
- `DEL-MVP-P17-02` — SLOs/alerts/dashboards; versioned, owned, reviewed and linked.
- `DEL-MVP-P17-03` — runbooks/on-call; versioned, owned, reviewed and linked.
- `DEL-MVP-P17-04` — incident/support model; versioned, owned, reviewed and linked.
