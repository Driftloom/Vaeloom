...
Do **not** assume `ENT-P10` passed because a document says “complete.” Re-audit its actual artifacts and evidence before executing this phase.
...
- `DEL-ENT-P10-05` — performance/deploy notes; versioned, owned, reviewed and linked.
...
On `NO-GO`, switch to `AUDIT_COMPLETED_PHASE` then `REMEDIATE_FAILED_PHASE` for `ENT-P10`. Re-run its full gate and generate a new handoff before resuming this file.
...
## 1. Mission
Implement core services with authorization, isolation, async processing, idempotency, integrity, audit and operations.
...
- **Phase-specific rule:** Implement control-plane/cell, policy, billing, lifecycle, audit, plugin gateway and bulk jobs.
...
6. Implement control-plane/cell, policy, billing, lifecycle, audit, plugin gateway and bulk jobs.
...
- Apply phase rule: Implement control-plane/cell, policy, billing, lifecycle, audit, plugin gateway and bulk jobs.
