...
> **Purpose:** Define IAM strategy for Vaeloom **Status:** ✅ Upgraded to
> enterprise quality **Owner:** Security Team **Last Updated:** 2026-07-13
...
graph LR
 ...
 direction TB
 I1["User<br/>Auth Provider"]
 I2["Agent<br/>Agent Registry"]
 I3["Service<br/>Service Mesh"]
 I4["Integration<br/>Plugin Manifest"]
 end

 subgraph AuthN["Authentication<br/>Who are you?"]
 A1["User: JWT from Native Auth Service"]
 A2["Agent: Internal registry ID"]
 A3["Service: mTLS certificate"]
 A4["Integration: Manifest signature"]
 end

 subgraph AuthZ["Authorization<br/>What can you do?"]
 ...
 subgraph Audit["Audit<br/>What did you do?"]
...
> **Diagram:** IAM model flows through 4 stages — **Identity Types** (User,
> Agent, Service, Integration) → **Authentication** (JWT/mTLS/manifest) →
> **Authorization** (MVP vs Enterprise levels) → **Audit** (quarterly/monthly
> reviews). Each identity type has its own authentication mechanism,
> authorization scope, and audit cadence.

## Identity Types

| Identity    | Source                                 | Purpose                          |
| ----------- | -------------------------------------- | -------------------------------- |
| User        | Native Auth Service (FastAPI + bcrypt) | End-user identity                |
...
This document defines the Identity and Access Management (IAM) strategy for
Vaeloom — covering identity types (User, Agent, Service, Integration),
authentication mechanisms, authorization levels (MVP and Enterprise), RBAC
roles, and access review schedules. Applies to all four identity types across
all environments. Out of scope: encryption (see
[Encryption.md](./Encryption.md)), secrets management (see
[Secrets.md](./Secrets.md)), compliance (see [Compliance.md](./Compliance.md)).
...
2. Native Auth Service (FastAPI + bcrypt) handles authentication
   (email+password, Google/MS OAuth, SAML SSO)
3. JWT issued with claims: user_id, tenant_id, exp (HS256 per `config.py:53-56`)
4. Access token (1h TTL) + Refresh token (30d TTL stored in Redis)
5. All subsequent requests include JWT in Authorization header and workspace ID
   in `X-Workspace-ID` header or URL path
6. TenantMiddleware validates JWT signature, verifies workspace membership in
   PostgreSQL, and sets RLS session GUCs
7. Every request checked against workspace scoping and RBAC permissions
...
sequenceDiagram
 ...
 participant AP as Auth Service (FastAPI)
 ...
 AP->>AP: Authenticate (bcrypt / SSO assertion)
 AP-->>U: JWT (user_id, tenant_id, exp) + Refresh token

 U->>APP: Request with JWT + X-Workspace-ID
 APP->>TM: Validate JWT + verify workspace membership
 TM->>TM: Bind PostgreSQL RLS GUCs (tenant_id, workspace_id)
 APP->>PE: Check action permissions (RBAC)

 PE->>PE: Verify user role within workspace
...
> **Diagram:** User authentication flow — Auth provider issues JWT with identity
> claims. Permission Engine validates and checks RBAC. Workspace_id is extracted
> from the token (not request body) to prevent tenant spoofing.
...
Vaeloom's Identity and Access Management (IAM) system governs user
authentication and authorization across the entire platform. The system uses
OAuth 2.0 + OIDC for authentication with Google/GitHub social providers and
email/password authentication via Supabase Auth. Authorization follows
Attribute-Based Access Control (ABAC) using JWTs containing role and permission
claims.

This document defines the authentication flow, role hierarchy, permission model,
API authorization patterns, and session management. The primary audience is
full-stack engineers implementing auth features and security engineers auditing
access control.

Within the Vaeloom platform, IAM ensures that every request — whether from a web
user, API client, or AI agent — is properly authenticated and authorized before
accessing any resource. The authorization model is resource-scoped: users can
access only their own documents and memories unless granted explicit
cross-resource permissions.

Enterprise-grade IAM requires a layered approach: authentication (who you are +
MFA), authorization (what you can do), and audit (what you did). Every access
decision must be logged with the actor, action, resource, and outcome for
...
- Authenticate all users via OAuth 2.0/OIDC with at least one social provider
  (Google/GitHub)
...
- Log every authorization decision with actor, action, resource, and outcome for
  audit trail
...
- User authentication: OAuth 2.0 + OIDC (Google, GitHub), email/password
  (Supabase Auth)
- Authorization model: ABAC with roles (user, admin, owner) and permissions
  (read, write, delete, admin)
- JWT structure and claims: sub, role, email, org_id, permissions array,
  session_id
- Permission enforcement in API middleware (FastAPI dependencies) and AI service
  (internal API key)
- Session management: refresh tokens, session revocation, secure cookie vs
...
- Fine-grained row-level security in PostgreSQL (covered in
  [Security-Architecture.md](./Security-Architecture.md))
- Audit log storage and querying (covered in [Audit-Logs.md](./Audit-Logs.md))
- API key management for external integrations (covered in
...
### Example 2: API Guard Implementation (Python)
...
async def require_permission(permission: str):
    """Dependency that checks required permission on every request."""
...
async def create_document(
    ...
    return await documents_service.create(body)
```
...
> **Diagram:** IAM flow — user authenticates via OAuth, receives JWT, every API
> request is verified (signature + expiry) and authorized (permission check),
