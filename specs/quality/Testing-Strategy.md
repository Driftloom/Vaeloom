...
> **Purpose:** Comprehensive testing strategy for Vaeloom — from unit tests to
> production monitoring **Status:** ✅ Upgraded to enterprise quality **Owner:**
> QA Team **Last Updated:** 2026-07-12 **Canonical source:**
...
Vaeloom's testing strategy follows a pyramid approach with the right balance of
speed, confidence, and cost. The strategy covers traditional software testing
(unit, integration, E2E) and AI-specific testing (golden datasets, adversarial
tests, evaluation framework).

This document defines the testing philosophy, the types of tests, coverage
targets, and how tests integrate with the CI/CD pipeline.
...
| **Smoke**       | Critical endpoints post-deploy   | < 30s total  | High           | curl + assertions  | Every deploy        | ✅ Implemented — `testing/smoke/README.md` 5 suites / 12 cases (health 2, auth 3, workspace 2, memory 3, agent 2); closes EXC-P14-04 partially (chaos/fuzz still empty → TODO)                                                        |
| **Load**        | System under expected load       | < 30 min     | High           | k6                 | Pre-release         | ✅ Implemented                                                                                                                                                                                                                        |
| **Security**    | Vulnerability scanning           | < 5 min      | High           | ZAP, Dependabot    | Weekly              | ✅ Implemented — `apps/api/tests/security/` 233 collected / 170 unique (middleware/test_csrf duplicates per zero-trust audit 2026-08-22 F-02); see `docs/phases/mvp-p13/09-gate-report.md` + `docs/phases/mvp-p14/05-test-results.md` |

> **WS-E 2026-09-15 verified:** backend `apps/api/tests/` 265 `.py` files (excl.
> `__pycache__`; 207 claimed 2026-08-23 now stale — recount 265), full-suite
> collect 2731 (was 2555 at `ea329dd`; xdist hang finding-39 — use
> `-o addopts="-n auto --dist loadfile"` ~2-3min/32GB or serial `-o addopts=""`
> ~8-10min), SQLite `tmp_path` per-test DB via `NullPool`, `mock_llm` +
> `mock_connector_test` autouse (`apps/api/tests/conftest.py`), coverage 94.2%
> (`mvp-p15/05-test-results`, retained not re-measured — EXC-P14-01 open),
> WCAG/k6 EXC-P14-02/03 open, chaos/fuzz code dirs empty → TODO.
...
describe('DocumentService', () => {
  ...
  describe('deduplicateDocuments', () => {
    ...
    it('should flag content-similar documents as duplicates', async () => {
      mockRepo.findById.mockResolvedValueOnce(null).mockResolvedValueOnce({
        id: 'doc_2',
        contentHash: 'abc123',
      });
...
test.describe('Document Upload and Organization', () => {
  ...
  test('user uploads resume and agent proposes organization', async ({
    page,
  }) => {
    // Upload document
    ...
    // Verify proposal
    await expect(
      page.locator('[data-testid="proposal-filename"]'),
    ).toContainText('Resume_2026');
    await expect(page.locator('[data-testid="proposal-folder"]')).toContainText(
      'Career > Resume',
    );
...
> **WS-E quarantine note 2026-09-15:** prior revisions of the Test Types table
> marked Smoke/Security `NOT_IMPLEMENTED (empty directory)`. That is STALE and
> superseded by the table above (smoke 5/12 inventory + 233-suite exist).
> Original lines retained in git history — do not treat as current.
